...
| ---: | --- | --- | --- | --- | --- |
| 0 | Readable current-location lookup follow-up (FR-17.2-adjacent profile and property-search UX; no completion-percentage change) | **Done — [PR #208](https://github.com/brollysolutions/client1/pull/208)** on `codex/20260819-165700-lets-work-on-dhanadhara-logo-and-dhanadhar`: **Use current location** now returns an editable city/locality in Profile and is available in the shared property search used across dashboard home, Explore, category, and Bookmarks surfaces. The free no-key BigDataCloud client endpoint is deployment-configured rather than hardcoded. Completion coverage remains 99.4%; next priority remains FR-2.2 controlled correction/audit. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | Two-decimal rounding happens before the credential/referrer-free outbound request; only normalized/bounded place fields reach Profile or URL-synced property filters; no coordinate fallback, background capture, history, or personalization coupling exists. Provider/permission/timeout/malformed failures preserve manual values, absent/invalid HTTPS configuration hides the action, and provider/IP disclosure is present. Operational pickup/address, authored listing, audience, and generic search fields remain unchanged; APIs, contracts, schema, auth/RLS, deletion, logs/audit, dependencies, and business-line isolation are unchanged. Fresh evidence: web lint/typecheck, 381 unit tests, focused registration plus authenticated property-search Playwright journeys at 390px, desktop/phone visual inspection, and security/design/maintainer review pass. A broader stale navigation test fails before this feature at `Compare Loan Offers`. Build compiles/typechecks/generates 93 pages before the known Windows standalone `EPERM` tail and Docker-only `api` host limitation. |
| 0 | Registration/profile location and salaried terminology (FR-17.2, FR-18.1-adjacent profile UX; no completion-percentage change) | **Done — [PR #207](https://github.com/brollysolutions/client1/pull/207)** on `codex/20260819-165700-lets-work-on-dhanadhara-logo-and-dhanadhar`: Postal address is replaced by optional Location in registration, Profile settings, the API, database, privacy notice, and generated contracts; Net salary/`net_salary` is replaced by Salaried/`salaried`. Completion coverage remains 99.4%, and the next priority remains FR-2.2 controlled correction/audit. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | Migration `e4b5c6d7e8f9` preserves address values by renaming the column and migrates the income literal; upgrade→downgrade→upgrade and one applied head pass. Current location is explicit, low-accuracy, two-decimal, editable, and saved only with the form; no reverse geocoder, background capture, history, or personalization coupling exists. RLS is unchanged, audit/log/token/Redis payloads exclude location, and deletion scrubs it. Fresh evidence: 53 focused API tests, API Ruff/format, byte-stable regenerated contracts, web lint/typecheck, 375 unit tests, and the focused desktop/390px registration Playwright journey pass. Broad Playwright is 11/15 with four unrelated stale-flow failures. The host build compiled/typechecked/generated 93 pages before the known Windows standalone `EPERM`; the Linux workaround and 13-minute monolithic API run are inconclusive. Security, design, and maintainer review found no remaining actionable defect. |
...
- **Compatibility and security:** browser navigation remains a UX hint only;
  API dependencies and PostgreSQL RLS remain authoritative. Preserve Client
  local-storage preference, dual-line switching, short-viewport navigation,
...
in PR #148. The direct 2026-08-20 user decision supersedes only those points:
the current profile uses optional Location and offers an explicit, two-decimal
current-location action without map/GMB or personalization coupling.
