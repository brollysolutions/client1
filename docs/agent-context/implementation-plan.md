...
**100%** across 79 active requirements; see
...
CI/Security/browser/contract gates pass; exact first-party builds and all six
authenticated private-runtime re-pulls pass fresh vulnerability, secret, SBOM,
runtime, and synthetic recovery checks. Release remains NO-GO because target
capacity, production environment/provider evidence, and all nine human-owned
decisions remain open. No deployment, registry mutation, production access,
provider/live-payout activity, capacity approval, or human sign-off occurred.
...
**Merged on `security/browserslist-4-28-7` — [PR #290](https://github.com/brollysolutions/client1/pull/290):** the
...
| ---: | --- | --- | --- | --- | --- |
| 0 | Local web port 3001 maintenance | **Done locally — [PR #295](https://github.com/brollysolutions/client1/pull/295)** on `feat/production-environment-evidence`: the development web entry point now uses host port 3001 so it can run beside another local application on 3000. | `gpt-5.6-terra` / Medium | `gpt-5.6-terra` / Medium | Docker Compose publishes `3001:3000`, retaining its in-container listener and health check on 3000; direct web commands, browser-test defaults, development API/CORS defaults, MinIO upload CORS, seed output, and developer documentation consistently use 3001. Focused API configuration/link tests, Compose rendering, and web lint/typecheck/unit tests pass. The web build compiles, typechecks, and generates 94/94 routes before the established Windows standalone-symlink `EPERM`. Production topology and internal container ports remain unchanged. |
| 0 | Production DNS/TLS, runtime-configuration, monitoring, and alert-delivery evidence gate (release blocker; no requirement expansion) | **Complete locally in [PR #294](https://github.com/brollysolutions/client1/pull/294); real-environment evidence remains NO-GO.** A provider-neutral runbook, intentional NO-GO example, and closed privacy-minimized assessor now cover the three canonical rows (`ENV-DNS-01`, `ENV-SEC-01`, and `ENV-MON-01`) without selecting a vendor or changing production. Fresh evidence passes 11 focused tests, all 86 script tests with one expected Windows skip, API Ruff/format over 504 files, one Alembic head, frozen web install, web lint/typecheck, and 95 files / 610 web tests. The native build compiles, typechecks, and generates 94/94 routes before the established Windows standalone-symlink `EPERM`; the full API run was stopped at 3% after repeated database-unavailable skips because Docker/WSL was unresponsive, so neither incomplete command is claimed as passing. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | The bounded closed-schema assessor binds an independently supplied exact candidate and frozen environment-manifest digest; requires complete external DNS/TLS/redirect/certificate/header/HSTS/CORS/proxy/object-policy/request-limit evidence; requires all-service runtime-configuration ownership, uniqueness, entropy, least privilege, access audit, rotation/rollback/expiry exercise, and zero default/orphan/browser-exposed private values without recording values or names; requires the complete monitoring/SLO/alert inventory, all intended delivery-route counts, primary/escalation delivery within approved targets, log/trace privacy review, current ownership/runbooks, and distinct gate-specific approvals after the run. Placeholder, duplicate, unknown, sensitive, partial, stale, failed, mismatched, or self-approved evidence returns NO-GO and receives no evidence digest. No domain/provider selection, credential handling, production query/mutation, DNS change, deployment, schema/API/RLS/payout change, or launch approval occurred. Named operators and qualified gate owners must still execute and approve the run against the exact real environment; next priority is that execution plus the remaining qualified human sign-offs. |
| 0 | Configurable 4-GB production profile and target-host capacity evidence gate (release blocker; no requirement expansion) | **Complete locally in [PR #293](https://github.com/brollysolutions/client1/pull/293); target evidence remains NO-GO.** The user approved the existing 2-vCPU / 4-GB droplet as a hard launch constraint while retaining PostgreSQL, Redis, ClamAV, PgBouncer, isolated FFmpeg, API, scheduler, web, and nginx. The implemented defaults replace 9.125 GiB of prior ceilings and four API workers with an overridable 3,488-MiB compact candidate, one API worker, bounded auth/request/media/database/scheduler/scan concurrency, and serialized offline ClamAV updates. Fresh local evidence passes 75 script tests with one expected Windows skip, API Ruff/format, 74 focused host tests with three skips and the affected 77-test database-backed slice, one Alembic head, compact Compose rendering, updater syntax, web lint/typecheck, and 610 tests. The native build reaches 94/94 routes before the established Windows `EPERM`; Docker Desktop became unresponsive during the correctly contextualized Linux web build and at 44% of the otherwise failure-free full database-backed API suite, so hosted CI remains required. Local no-swap ClamAV health and tested one-shot update probes pass. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | Production Compose supplies conservative, overridable worker, connection-pool, process-concurrency, and memory ceilings without removing either processor or weakening the quarantine -> scan -> sanitize/transcode -> ready boundary. ClamAV update/scan concurrency and FFmpeg work remain bounded; unavailable processors fail closed. The privacy-minimized evidence gate rejects the wrong host shape, missing required services/workloads, skipped malware/media/auth/scheduler/database/web scenarios, unhealthy/restarted/OOM-killed containers, excessive sustained memory/swap pressure, or absent reviewer approval. The checked-in example intentionally remains NO-GO. No deployment, production access/data, provider choice, schema/contract/RLS, payout, or feature removal occurred. A named operator and different reviewer must run and approve the workload on the exact Ubuntu `linux/amd64` target before this release blocker closes; larger-host overrides also require a new rehearsal. Next priority is that operator run, then production DNS/TLS/secrets/monitoring and qualified sign-off evidence. |
...
| 0 | Recover stalled hosted catalogue-filter navigation (release-gate defect; no requirement expansion) | **Complete locally - [PR #284](https://github.com/brollysolutions/client1/pull/284)** on `fix/catalogue-filter-navigation-recovery` from merged PR #283 commit `1e4181ee1f7983a4404eb59c712cc08ce25efec9`. Exact-candidate CI run `33493090662` passed the full repository gate, Linux production build, and registration journey, but catalogue filtering again delivered trusted input and received HTTP 200 RSC headers before the same-route `router.replace` stream remained uncommitted until the test closed. Four consecutive hosted runs reproduced the failure while repeated local `next start` runs passed. The repair retains App Router replacement as the fast path and, after a bounded eight seconds without URL commit, performs a same-origin full-document replacement and restores a short-lived scroll coordinate. The recovery record contains no query text or other user data. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | URL-based server filtering, 300ms debounce, progressive-enhancement GET form/links, sticky position, focus, live results, and zero retries remain intact. A deterministic browser regression stalls the filtered RSC request for ten seconds, proves fallback URL/results and scroll recovery, then proves clear-filter navigation; delayed and normal four-test release journeys pass 4/4. Web lint, strict typecheck, 94 files / 608 tests, and 44 tracking tests with one expected Windows skip pass. The controlled build compiles, type-validates, and generates 94/94 routes before the established Windows standalone symlink `EPERM`. API/auth/RLS/schema/contracts/business-line/PII/data/money/dependencies/deployment/registry/approval boundaries remain unchanged. Merge-result hosted Linux standalone CI must prove the fix before the next exact-candidate frozen-release rehearsal. |
| 0 | Repair the two proven hosted production-browser root causes (release-gate defect; no requirement expansion) | **Complete locally - [PR #283](https://github.com/brollysolutions/client1/pull/283)** on `fix/release-gate-root-causes` from merged PR #282 commit `f2dd580d998cd9bca2a1f53b5a09b6dca16a65c8`. Merge-result CI run `33486705171` passed the full repository gate and retained the decisive failure bundle: registration delivered trusted input/click/submit events but CSP blocked the production client's fallback `http://localhost:8000` request under `connect-src 'self'`; catalogue delivered the trusted input event and began `/loans?q=insurance`, but its RSC response aborted before URL/result commit while the persistent public header's speculative RSC/chunk prefetch burst remained in flight under direct HTTP/1.1. Production now defaults an unconfigured browser API base to the documented same-origin nginx path while retaining explicit validated API origins and the non-production localhost fallback. Persistent public-header links remain real, keyboard-accessible links but no longer launch speculative prefetch; catalogue debounce/transition and server filtering are unchanged. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | Focused red/green coverage and adjacent API/CSP checks pass 15 tests; full web lint, strict typecheck, and 93 files / 606 tests pass. The controlled source build compiles, type-validates, and generates 94/94 routes before the established Windows standalone symlink `EPERM`; its normal `.next` production output passes all four zero-retry release journeys both with the 1.5-second delay and without it. All 45 script tests pass with one expected Windows POSIX skip, and design/security/maintainer review find no change-owned issue. API endpoints/responses, auth/RLS, schema/migration/contracts, business-line, PII/KYC, upload, money, dependencies, deployment, registry, approvals, and release NO-GO remain unchanged. Merge-result hosted CI must prove the exact Linux standalone gate; once green, the next engineering priority is a new exact-candidate frozen-release rehearsal while external and human sign-offs remain open. |
| 0 | Preserve decisive hosted evidence for the residual production-browser failure (release-gate diagnostic; no requirement expansion) | **Complete locally - [PR #282](https://github.com/brollysolutions/client1/pull/282)** on `test/release-gate-hosted-diagnostics` from merged PR #281 commit `15b510d90f4284add089ff16704cf3e41ff8e6c8`. Main CI run `33478724802` passed the full repository gate, then reproduced the same catalogue navigation and registration initiation failures after the controls reached the explicit enabled/`aria-busy` boundary. The workflow now retains the failure-only browser bundle through signature-verified, exact-pinned `actions/upload-artifact` v7.0.1 for seven days. Bounded test-only probes record event type, field name/value length, busy/disabled state, element removal, console/page errors, and sanitized request outcomes without values, bodies, headers, tokens, or response bodies. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | The upload contract fails before implementation and passes afterward; full web lint, strict typecheck, 91 files / 602 tests, four-test release enumeration, a deliberate failure bundle containing JSON/screenshot/error context/trace, a delayed fresh-build registration journey, all 45 script tests with one expected Windows POSIX skip, and `git diff --check` pass. The native build compiles, type-validates, and generates 94/94 routes before the established Windows standalone symlink `EPERM`. The catalogue journey remains unverified locally because Docker is unavailable and the host artifact cannot resolve the isolated fixture; the next merge-result hosted run must supply the retained evidence. Zero retries, the 1.5-second delay, original assertions, application/API/auth/schema/RLS/business-line/data/money/runtime behavior, and release NO-GO remain unchanged. Next priority is reading that artifact and implementing only the proven root fix. |
| 0 | Repair the residual production-artifact interaction-readiness failure (release-gate defect; no requirement expansion) | **Complete locally - [PR #281](https://github.com/brollysolutions/client1/pull/281)** on `fix/release-gate-interaction-ready` from merged PR #280 commit `f0524fa1fa177d1d044b211f08eacbb109d5c0c7`. Main CI run `33464102303` proved that focus and pressed state could become observable before the catalogue's controlled input and registration's initialized first step were reliably interactive. The repaired controls now remain disabled and `aria-busy` until their client initialization commit completes; the delayed release journeys enter at response commit and wait for that explicit enabled boundary. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | Final-source changed-file ESLint, strict typecheck, 91 files / 602 tests, 44 script tests with one expected Windows POSIX skip, `git diff --check`, and strict Linux image `085d12ec2b28...` building/exporting all 94 routes pass. The preceding exact image passed both delayed and normal no-retry journeys at 4/4; the final image's post-export browser rerun is not claimed because Docker's API crashed after C: reached zero free bytes. The only intervening source reduction removed an unreachable component-level no-JavaScript fallback discovered during design review. No API/auth response, endpoint, schema/migration/contract, auth/RLS, business-line, PII/KYC, upload, payout/webhook, dependency, runtime, registry, deployment, or approval change is in scope. Hosted merge-result CI remains required and launch remains NO-GO. |
| 0 | Repair production-artifact browser hydration synchronization (release-gate defect; no requirement expansion) | **Merged in [PR #280](https://github.com/brollysolutions/client1/pull/280), but hosted defect persists.** The local controlled probe and repeated exact-image runs passed, but merge-result CI run `33464102303` disproved the readiness assumption: both focus and pressed state became observable before the tested controlled interaction paths were reliable on the hosted runner. The bounded 1.5-second chunk delay and zero-retry gate remain useful regression inputs; the new follow-up item above owns the root-cause repair. | `gpt-5.6-terra` / High | `gpt-5.6-terra` / High | Historical evidence only: delayed local hydration passed 12/12, normal production-image journeys 4/4, final rebuilt strict Linux artifact 4/4, web lint/typecheck/602 tests/build, and 44 script tests. Hosted merge evidence failed 2/4 after full verification passed, so this row is not release-complete and the exact-candidate rehearsal remains blocked. |
| 0 | Remove the obsolete Python-audit exception and enforce API/scheduler/web runtime least privilege (release hardening; no requirement expansion) | **Complete locally — [PR #279](https://github.com/brollysolutions/client1/pull/279)** on `security/api-web-runtime-hardening` from merged PR #278 commit `46f43d3385cbb112d7881c852c4a09d64cafab05`. The stale `PYSEC-2026-1325` suppression and rationale are gone; the frozen production lock is clean with no allowlist. Production API, scheduler, and web now enforce fixed UID 100/GID 101 non-root identities, read-only roots, all-capability drops, no-new-privileges, init, PID bounds, and only explicit bounded `noexec,nosuid,nodev` `/tmp` mounts. Next ISR revalidation and image optimization use the explicit bounded memory cache without disk flush. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | Fresh evidence passes 11 production-runtime contracts and all 44 script tests (one expected Windows POSIX-resource skip), both Compose renders, a no-exception Python audit, the production web audit, API Ruff/format over 504 files, one Alembic head, API/scheduler import and write-boundary probes, web lint/typecheck, 91 files / 602 tests, a strict 94-route Linux image, 4/4 no-retry production-artifact journeys, clean repeated stale ISR and image-optimizer requests, and 0 High/Critical Trivy rows for both final application images. Root writes fail for all three application services while only `/tmp` and the scheduler heartbeat work as declared. Endpoint, auth/RLS/business-line, PII/KYC, upload, payout/webhook, cookie, schema/migration, generated-contract, application behavior, registry, and human-approval boundaries remain unchanged. Deployment, publication/re-pull, provider transfer, live payout, production environment proof, and human sign-off remain non-goals and release blockers. |
| 0 | Exact-candidate frozen-release rehearsal after container CVE remediation (release blocker; no requirement expansion) | **Rehearsed — NO-GO — [PR #278](https://github.com/brollysolutions/client1/pull/278)** on `chore/exact-candidate-rehearsal-post-cve` at exact merged PR #277 candidate `eefc61d06708635f79055fe0187ede4fed3185cf`. Every locally obtainable gate has a terminal result; missing hosted, registry, production, external-provider, and human evidence is not waived. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | [`release-rehearsal-2026-08-30.md`](release-rehearsal-2026-08-30.md) records all nine exact application/service images at 0 High/Critical, 1,948/1,948 API tests, 602 web tests and a 94-route build, 4/4 exact-image browser journeys, clean stricter dependency/secret audits, exact API/web/media probes, and matching timed synthetic database/object restores. Release remains blocked by zero-step hosted jobs, absent approved registry publication/re-pull, approved-provider transfer/live-payout limits, unproved production recovery/edge/secrets/monitoring, and all nine human sign-offs. The obsolete `PYSEC-2026-1325` workflow ignore and the fact that production Compose does not enforce read-only roots for API/web are explicit follow-ups. No production secret/data, deployment, live payout, registry publication, risk acceptance, or inferred approval occurred. Next engineering priority is to remove the stale audit exception and decide/test writable directories before the next exact candidate; operational priority is restoring hosted execution and supplying approved registry and production/human evidence. |
| 0 | API and isolated media-runtime high/critical container remediation (release blocker; no product requirement expansion) | **Complete locally — [PR #277](https://github.com/brollysolutions/client1/pull/277)** on `security/container-cve-remediation` from merged PR #276 commit `d9edc53270c5ed7f0c2ded593e76b7a334149822`. Digest-pinned Alpine runtimes, exact fixed OpenSSL/SQLite/FFmpeg packages, a build-only pinned `uv`, and PyJWT in place of the unused ECDSA dependency remove every rehearsed High/Critical row without an allowlist or engineering acceptance. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | Final API image `sha256:6a95bef6bc2451f1d0376dbde1bc5f779f15cbd6702a4e7cd8c610facce4a25d` is non-root, excludes `uv`, FFmpeg, and the Jose/ECDSA chain, and has 0 High/Critical Trivy rows, 0 secret findings, and 123 CycloneDX components. Final media image `sha256:4aee6affa6fe0a384218f36761e63a9ae3d1fadde83a038213d84e7c3182d064` retains the secretless/non-root/read-only/resource-bounded topology and has 0 High/Critical rows and 132 components; constrained 640x360 and policy-maximum 1920x1080 H.264/AAC round-trips and health pass. A fresh isolated Linux database run passes 1,948/1,948 API tests without skips or retries; all 504 API files pass Ruff/format, JWT negative cases pass, one Alembic head remains, 34 structural tests pass with the expected Windows-only skip, web lint/typecheck and 91 files / 602 tests pass, and frozen API/web dependency audits are clean (the documented pip advisory exception remains unchanged). No registry publication, production access, schema/contract/RLS change, live upload/payout, or human approval occurred. Next priority after merge is a new exact-candidate frozen-release rehearsal, final registry publication/re-pull scanning, and the still-open production/environment and nine human gates. |
| 0 | Zero-cost exact-candidate frozen-release rehearsal after merged API and Playwright repairs (release blocker; no requirement expansion) | **Rehearsed — NO-GO — [PR #276](https://github.com/brollysolutions/client1/pull/276)** on `chore/frozen-release-rehearsal-final` at exact merged PR #275 candidate `fb692260c4793562b49915e740fbac09dd893b6d`. Every locally obtainable gate has a terminal result; missing hosted execution and production/human evidence are not waived. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | [`release-rehearsal-2026-08-30.md`](release-rehearsal-2026-08-30.md) records 1,944/1,944 API tests, 4/4 exact-image browser journeys, 602 web tests and a 94-route build, current secret/dependency/container scans, exact runtime headers/CORS/auth/upload/webhook/media probes, and matching timed synthetic database/object restores. Release remains blocked by zero-step hosted jobs, API/media high/critical findings without fixes or acceptance, absent final registry references and approved external upload, unproved production recovery/edge/secrets/monitoring controls, and all nine human sign-offs. No production secret/data, live payout, image publication, deployment, gate weakening, or inferred approval occurred; essential-only cookie treatment remains unchanged. |
| 0 | Production-artifact Playwright release gate (release blocker; no requirement expansion) | **Done - [PR #275](https://github.com/brollysolutions/client1/pull/275)** on `test/playwright-release-gate`: the CI gate now launches the standalone production server against a loopback-only, contract-shaped catalogue fixture, fails closed when the artifact is absent, and runs without retries. Exact section locators tolerate legitimate campaign copy, internal enquiry links remain root-relative, and mocked registration now mirrors the authenticated-layout refresh handoff. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | Web lint, strict typecheck, and 91 files / 602 unit tests pass. The fixture-backed production server passed 4/4 before the launcher was tightened; the final fail-closed launcher passes Node syntax checks and enumerates exactly four tests. Three repeated exact-image runs pass 12/12 against 12 live catalogue cards, including the formerly flaky registration handoff. The pre-change strict image reproduced both stale assertions and built all 94 routes; the post-change image reinstall passed the frozen 645-entry supply-chain check but Docker stopped responding during compilation, so its export and final standalone-gate execution are not claimed. No application UI, API, generated contract, auth/RLS, cookie, upload, money, schema, migration, dependency, or public workflow changed. Next priority is restoring hosted billing and rerunning the complete frozen-release rehearsal on both merged repairs. |
| 0 | API frozen-release baseline repair (release gate; no requirement change) | **Done - [PR #274](https://github.com/brollysolutions/client1/pull/274)** on `test/api-release-baseline`: the 11 reproducible stale policy, invalid property-UUID fixture, and validation-order failures plus two aggregate-order defects from the exact-candidate rehearsal are corrected without changing production behavior. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | A freshly migrated direct-PostgreSQL Linux run reports one `d9f1a3b5c7e0` Alembic head, all 503 files passing Ruff/format, and 1,944/1,944 API tests passing in 58:09. Platform-Admin CMS authority, Sub Admin ownership, non-enumerating 404s, canonical active-property resolution, vehicle ownership/assignment RLS, notifications, payout grace, and retry-pool behavior remain covered. No endpoint, contract, product workflow, authorization rule, RLS policy, migration, or dependency changed. Next priority is the production-artifact Playwright release gate. |
| 0 | Exact-candidate frozen-release rehearsal after immutable-image remediation | **Rehearsed — NO-GO — [PR #273](https://github.com/brollysolutions/client1/pull/273)** on `chore/frozen-release-rehearsal` at merged PR #272 candidate `c37b9d5`. Exact local build/scan/runtime/CORS/auth/upload-presign/webhook and timed synthetic database/object restore evidence is recorded without accepting blocked, failed, or external gates. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | [`release-rehearsal-2026-08-30.md`](release-rehearsal-2026-08-30.md) records the terminal results. The 93-test security set, web 602-test/build gate, exact headers/CORS/auth roles, negative webhook, private presign, and local restores pass. Release remains blocked by hosted billing, API 14 high / 3 critical and media 137 high / 7 critical rows, absent final registry hashes, 11 reproducible stale/invalid API tests, failed/flaky Playwright evidence, no positive approved-provider upload, no production recovery/edge/secrets/monitoring evidence, and every qualified human approval. |
| 0 | Frozen-release rehearsal and go/no-go evidence (pre-deployment checklist) | **Rehearsed — NO-GO — [PR #268](https://github.com/brollysolutions/client1/pull/268)** on `chore/frozen-release-rehearsal` at frozen candidate `3cc6bc0` (merged PR #267). The evidence report fixes frozen dependency auditing, vulnerable locked Python packages, the broken production API command, and the vulnerable unused web package-manager payload, while keeping interrupted and external gates explicitly unpassed. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | Evidence is recorded in [`release-rehearsal-2026-08-30.md`](release-rehearsal-2026-08-30.md). Host API style/tracking, one Alembic head, generated contracts, web lint/typecheck/602 tests/build, Gitleaks, frozen dependency audits, non-root artifact checks, web headers, app SBOMs, and image scans have fresh results. Launch remains blocked by hosted-CI billing, unaccepted API/service-image findings, mutable image tags, Docker-engine failure during the aggregate/runtime/restore phase, absent real DNS/TLS/secrets/backups/monitoring evidence, and every named human sign-off. No consent banner is appropriate while storage remains essential-only; Accept/Reject is required before non-essential storage where consent applies. |
| 0 | Credentialed browser CORS method/header hardening (frozen-release blocker slice) | **Done - [PR #270](https://github.com/brollysolutions/client1/pull/270)** on `security/cors-policy`. The API now grants only the five methods and three non-safelisted request headers used by the centralized browser client, explicitly exposes the report truncation response header, and rejects wildcard, opaque `null`, userinfo, malformed-port, non-HTTP(S), and non-origin CORS configuration. Exact configured origins and credentialed refresh-cookie/Bearer behavior are preserved; direct-to-storage multipart POSTs remain under the storage provider's separate CORS policy. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | Fresh positive/negative evidence passes all 48 CORS/config tests on the host and the pre-final 42-test set in the Linux API image, including representative refresh, Bearer, business-line, JSON upload-presign, and report-download preflights; arbitrary, `null`, suffix-confusion, HEAD/TRACE, and invented-header probes receive no grant. All 503 API files pass Ruff/format, one Alembic head plus 11 migration/RLS and 4 production-runtime tests pass, and web lint/typecheck plus all 91 files / 602 tests pass. The fresh Linux aggregate completes 1,911 passes / 13 failures; an exact rerun on a newly migrated database makes the two order/state-sensitive failures pass and reproduces 11 stale content-policy, UUID fixture, and validation-order expectations, with no CORS/config failure. The strict Linux web production image compiles, typechecks, generates 94/94 routes, completes standalone copy, and exports. The Bash repository wrapper was unavailable because this Windows host has no WSL distribution; its available constituent gates were run directly. API routes/contracts, auth/RLS, cookies, uploads, and dependencies are unchanged. Remaining evidence is a real deployed browser/proxy/storage-edge probe against the exact release candidate; next priority is the remaining frozen-release blocker remediation and exact-candidate rerun. |
| 0 | Frozen-release blocker remediation and exact-candidate rerun | **Native-media slice done — [PR #271](https://github.com/brollysolutions/client1/pull/271) on `security/media-runtime-isolation`; the release remains NO-GO.** Attacker-controlled FFprobe/FFmpeg execution is removed from the credentialed API/scheduler image and runs in a secretless, non-root, read-only, resource-bounded worker on an internal network. Only the scheduler joins `media-control`; the API merely validates the shared production origin and has no call path or network route to the worker. The scheduler retains database, storage, ClamAV, locks, state, retry, cleanup, and retention ownership. Hosted-runner recovery, other service-image replacement/digest pinning, the exact-candidate rerun, recovery exercise, and human/environment sign-offs remain separate blocker work and the next priority. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | **Exit evidence:** the worker base is digest-pinned; production origin validation fails closed; runtime contracts assert no credentials, public/data-service route, volumes, capabilities, or root writes plus explicit CPU, 768 MiB real-memory, 1.25 GiB child address-space, PID/NPROC, wall/CPU, tmpfs, file-size, allocation, thread, and descriptor limits. A hardened Linux run passed 7/7 unit tests, a real 1920x1080 H.264/AAC canonical round-trip, five sequential transcodes with four concurrent health probes (all HTTP 200, max probe 21 ms), and stable Docker health. Focused API tests passed 61; production contracts 7; feature tracking 7; migration/RLS tracking 11; Ruff/format, one Alembic head, contract drift, Compose rendering, web lint/typecheck, and 91 files / 602 web tests passed. The Linux API aggregate completed 1,899 passes / the same 13 unrelated baseline failures. Exact worker digest `sha256:2646e443e722b471149ee63359dbad253f36c0f1e06d1fca1c60110907e2d403` has a 294-component CycloneDX SBOM; Trivy 0.74.0 reports 137 high rows / 7 critical rows / 45 unique advisory IDs across 24 packages, with no fixed version reported. Those findings are contained, not removed, waived, or claimed unreachable; container/kernel escape and worker availability remain residual risks. Upload contracts, ownership/RLS, private staging, fail-closed ClamAV, canonical replacement, quotas, idempotency, deletion, cleanup, and retention remain unchanged. Image/PDF processing, scanner policy, formats/limits, schema/contracts, broad image remediation, and advisory allowlisting remain non-goals. Security and maintainer review found no remaining change-owned actionable defect. |
| 0 | Frozen-release blocker remediation and exact-candidate rerun | **Human evidence register prepared — approvals remain open — [PR #269](https://github.com/brollysolutions/client1/pull/269), [`launch-signoff-register.md`](launch-signoff-register.md).** This bounded documentation slice used `gpt-5.6-terra` / Medium; the canonical register covers trademark, entity/Terms, privacy/processors, DNS/TLS, secrets, monitoring, and incident ownership while the technical blocker PRs proceed separately. Each of nine stable gates names its accountable role, required evidence, decision values, approver/date fields, and expiry rules; no unchecked item or empty approver is approval. The register is not legal advice, engineering self-approval, production evidence, technical remediation, or a change to the essential-cookie decision. It links approved evidence locations without storing secrets, production exports, customer data, private contracts, or privileged legal material in Git. Fresh evidence: all changed relative Markdown links resolve, all 7 feature-tracking tests pass, `git diff --check` passes, and security/maintainer review found no actionable issue. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | Accountable humans can now record Approved / Rejected / Time-bounded risk accepted decisions with dated evidence and mandatory expiry/remediation where allowed. Every gate and the final release decision remain Open/NO-GO. Next: obtain the named approvals while the separate immutable-image PR lands; then run the exact-candidate production-artifact, critical-journey, timed-recovery, and environment-evidence rehearsal. |
| 0 | Frozen-release blocker remediation and exact-candidate rerun | **Runtime-image slice refreshed through merged PR #271 — [PR #272](https://github.com/brollysolutions/client1/pull/272) on `security/runtime-image-pinning`; release remains NO-GO.** All six production service images now have versioned release inputs and mandatory final registry-manifest hashes; production cannot fall back to building the media worker. The five existing outputs remain zero-high/zero-critical. PR #271's exact worker artifact has a 294-component SBOM and 137 high / 7 critical rows with no reported fix; isolation is preserved but is not release acceptance. A fresh no-cache worker build produced `c18d048c4111…`, but a full C: drive and unresponsive Docker backend prevented its scan/SBOM/smoke, so it is explicitly unreviewed and unusable. Fresh host evidence runs 34 script/runtime/tracking tests (33 pass, one expected Windows skip), passes 54 focused API tests with one Linux-only skip, Ruff/format over 503 files, one head, both Compose renders including missing-media-digest rejection, web lint/typecheck, 602 tests, and compile/typecheck/94-route generation before the known Windows symlink `EPERM`. No registry publication or advisory acceptance was authorized. Next priority: restore Docker capacity, publish and re-pull exact reviewed six-image manifests, complete scan/SBOM/runtime and full frozen-candidate CI/journey/restore evidence, then obtain the still-open environment and human sign-offs. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | CI and Security execute on the exact reviewed SHA; all six production service images are immutable and every reachable critical/high finding is remediated or has specific accountable, expiring acceptance; full API/web/Playwright gates complete with owned failure disposition; exact allowed and arbitrary/null/suffix CORS probes pass; production-artifact auth, role, upload, webhook/payout, and recovery journeys pass; isolated database/object restore records reconciled integrity and achieved RPO/RTO; real edge/secret/monitoring evidence and all accountable human approvals are linked before the final go/no-go. |
| 0 | Admin controlled-correction and append-only operational audit remediation (FR-2.2) | **Done — [PR #267](https://github.com/brollysolutions/client1/pull/267)** on `codex/20260829-163717-implement`. The original approved-listing correction gap and seven named audit families are closed. The exhaustive registry then identified two newer read-only visibility gaps; PR #286 and the current minimized financial-enquiry delivery close both, so this earlier row no longer blocks FR-2.2 completion. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | A platform Admin can stage a full typed, reason-required correction against an approved property submission; the live listing and approved media remain unchanged until RERA re-review and approval update the existing catalogue row. Banner, content-block, loan-application, offer, property-deal, referral-rule, and task mutations have append-only, same-transaction, PII-safe business audit evidence. Existing role, ownership, business-line, RLS, immutable-ledger, document/media, payout, and protected secret/location boundaries remain unchanged. Fresh evidence: all 502 API files pass Ruff/format; the final affected set passes 174 tests; the aggregate API suite completes 1,881 passes, 15 unrelated baseline failures, and 2 skips; migration round-trip and one Alembic head pass; generated contracts are current; web lint, typecheck, and all 91 files / 602 tests pass; the strict Linux production image completes with 94 routes; and security, design/accessibility, and maintainer review report no change-owned issue. The frozen-release rehearsal and human launch sign-offs remain separate next phases. |
| 0 | Pre-deployment authentication and session assurance: login enumeration resistance and bounded password-reset capability lifetime (direct user instruction; no formal requirement or completion-percentage change) | **Done - [PR #266](https://github.com/brollysolutions/client1/pull/266)** on `codex/20260829-163717-implement`. Unknown-mobile login now performs the same bounded Argon2 verification, identifier/IP failure accounting, nullable-subject security-event write, and generic rejection as a known-account bad password. Password-reset capabilities now use the authentication design's fixed ten-minute lifetime instead of inheriting the general access-token lifetime. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | Reset capabilities retain signed purpose/mobile claims, atomic single-use enforcement, suspended/deleted-account rejection, and password-triggered access/refresh revocation. Refresh rotation/reuse-chain revocation, secure host-only cookie attributes, server-side role/RLS enforcement, API response shape, and auth UI behavior are unchanged. The secure-cookie regression now asserts `HttpOnly`, `Secure`, `SameSite=Strict`, exact refresh path, and no `Domain`. A stale mobile-change assertion now proves both generation advances (mobile replacement and the password reset already exercised by the scenario). Fresh evidence: the two focused regressions failed before implementation and pass afterward; the Docker-backed auth directory passed 267 tests before the final audit/rate-symmetry follow-up, whose final direct service tests pass; the adjacent mobile-change regression passes; all 501 API files pass Ruff/format. The aggregate API run completed 1,877 passes / 16 failures in 74 minutes; the one adjacent stale assertion is corrected and green, leaving the same 15 unrelated stale/shared-state baselines documented by PR #259. Web lint, strict typecheck, and 91 files / 601 tests pass; the native production build compiles, typechecks, and generates 94/94 pages before the established Windows standalone-symlink `EPERM`. Feature tracking (7), migration/RLS tracking (11), the base-ref co-change gate, and one Alembic head pass. A Linux image retry became infrastructure-inconclusive when Docker Desktop's engine stopped responding. Security and maintainer review found no remaining actionable issue. Non-goals remain MFA/SSO, identity or role redesign, RLS/migrations, dependencies, and auth-screen redesign. |
| 0 | Shared dashboard interaction motion, reduced-motion behavior, and keyboard bypass navigation (continuation of the direct pre-deployment instruction; no formal requirement or completion-percentage change) | **Done - [PR #265](https://github.com/brollysolutions/client1/pull/265)** on `codex/20260829-163717-implement`. The public shell already had a first-focus skip path; the authenticated shell now provides the same bypass, and shared sidebar, cards, buttons, dialogs, drawers, menus, selects, tooltips, tabs, accordions, and popovers use one restrained interaction language with reduced-motion equivalents. The shared-boundary change reaches every role without a page-by-page rewrite. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | Every role dashboard exposes an app-first “Skip to main content” target; structural motion uses 200ms or less, direct feedback uses 150ms, transition properties are explicit, and reduced-motion users retain the same state changes without spatial animation. Keyboard focus, mobile navigation, active-route semantics, server-side authorization, API contracts, data handling, and role/business-line behavior remain unchanged. Fresh evidence: focused red/green interaction contract (4 tests); web lint and strict typecheck; 91 files / 601 unit tests; focused Playwright Client dashboard journey covering visible bypass focus/transfer, reduced-motion sidebar CSS, 390px drawer navigation, close behavior, and no horizontal overflow; native production compile/typecheck/94-page generation before the established Windows standalone-symlink `EPERM`; and a strict Linux production image build through standalone copy and image export. Design/accessibility and maintainer review found no actionable change-owned issue. |
| 0 | Pre-deployment public identity, privacy/cookie transparency, design-token accessibility, browser security headers, and launch evidence (direct user instruction; no formal requirement or completion-percentage change) | **Done - [PR #264](https://github.com/brollysolutions/client1/pull/264)** on `codex/20260829-163717-implement`. The public shell now presents Dhanadhara with a visible shared wordmark, production configuration refuses missing/placeholder public identity and insecure external origins, the legal surface accurately discloses essential cookies without an accept-only banner, and centralized headers plus shared semantic colors have regression gates. The operator checklist keeps unverified legal, TLS, backup/restore, monitoring, secret, and incident claims outside “code complete.” | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | Public identity consistently says Dhanadhara; production configuration rejects placeholder or non-HTTPS public values; an accurate legal notice inventories only the essential authentication cookies/storage currently used and explains why no consent banner is shown before non-essential tracking exists; shared legal navigation and sitemap expose it; security headers are centralized and regression-tested, with HSTS emitted only for production HTTPS delivery; shared semantic colors meet WCAG AA for normal text on white and cream; public navigation has a keyboard skip path; and a pre-deployment runbook separates code-complete controls from operator-owned TLS, legal, backup/restore, monitoring, secret, and launch gates. No auth, RLS, payment, upload, analytics, consent, or dashboard workflow behavior changes in this slice. Fresh evidence: web lint and strict typecheck pass; all 90 files / 597 tests pass; the native production build compiles, typechecks, and generates 94/94 pages before the established Windows standalone-symlink `EPERM`; the Linux production image builds completely from strict non-placeholder public configuration and includes the standalone runtime; desktop/390px Playwright review reports HTTP 200, no console error or horizontal overflow, first-focus skip navigation, and reduced-motion-safe announcement behavior. |
| 0 | Rent/lease listing intent and host-allowlisted external listing links (direct user instruction; no requirement or completion-percentage change) | **Done - [PR #263](https://github.com/brollysolutions/client1/pull/263)** on `claude/20260829-133027-rent-for-properties-link-option-while-list`. The real-estate catalogue was sale-only: no column separated a listing offered for sale from one offered to rent, `format_inr_display` rendered every price as lakh/crore, and browse carried no transaction facet. Separately, agents had nowhere to point a buyer at the property's existing presence on YouTube/Instagram/Facebook, because `_normalize_public_text` rejects every URL in property free text. | `claude-opus-5` / High | `claude-opus-5` / High | `re_listing_intent` (`sale`, `rent`, labelled "Rent / Lease") is a NOT NULL column on `properties` and `property_submissions`, server-defaulted to `sale` so every existing row backfills exactly. `price_paise` carries the headline amount for both intents, keeping every existing sort/filter/index working, and `format_inr_display` branches on intent so rent renders `₹25,000/month` instead of `₹0.25 L`. Deposit, minimum lease term, and availability date are rent-only columns that the schema layer rejects on a sale listing, so a draft switched back to Sale cannot keep advertising a deposit. `sale_type` becomes optional on the three detail families and is required for sale, forbidden for rent. Listing links are one capped JSONB column whose platform is DERIVED from a host allowlist rather than author-supplied, so a phishing host cannot wear a YouTube badge; the render path re-resolves every stored link. The narrative link ban is unchanged and still asserted. |
| 0 | Withdraw the campaign phone preview (direct user instruction; no requirement or completion-percentage change) | **Done - [PR #262](https://github.com/brollysolutions/client1/pull/262)** on `claude/20260829-131005-remove-phone-preview-as-of-now`. The phone preview added in PR #261 is withdrawn for now. With one width left there is nothing to switch between, so the size control is removed rather than rendered with a single option. | `claude-opus-5` / Medium | `claude-opus-5` / Medium | `ViewportFrame` renders at the single `DESKTOP_VIEWPORT_WIDTH` and no longer takes a viewport; `VIEWPORT_WIDTHS`, `ViewportName`, `viewportLabel` and `PreviewDevice` are gone, along with the device state in the banner wizard, offer form, banner queue and offer queue. Both preview panels state the width as static text. The seam for restoring a phone preview is documented in `viewport-frame.tsx`. |
| 0 | Campaign authoring follow-up: in-app confirmations, full-width workspaces, two-viewport preview, DhanaDhara preview branding, and an actionable approvals desk (direct user instruction; no requirement or completion-percentage change) | **Done - [PR #261](https://github.com/brollysolutions/client1/pull/261)** on `claude/20260829-122607-the-current-ui-is-good-for-banner`. Six reported defects after PR #260: the banner workspace stopped at `max-w-5xl` and left an empty band down the right of the full-screen dialog; the preview carried a third viewport that sat between two sizes already bracketing the layout; the phone preview rendered unscaled and left-aligned in a wide white panel with no device framing; every destructive confirmation used `window.confirm`, which Chrome renders as browser chrome pinned to the top of the window rather than over the workspace that asked; the preview chrome carried the `Logo` component's "Loans & Real Estate" wordmark instead of the DhanaDhara brand; the offer preview rendered a lone `max-w-md` card in an invented content area; and the approvals desk opened on every campaign ever created with nothing marking the few that needed a decision. | `claude-opus-5` / High | `claude-opus-5` / High | `useConfirm` replaces all twelve `window.confirm` call sites across six views with an in-app dialog, and a source-scanning test keeps them replaced. `DashboardFormPage` gains a `wide` mode so the banner wizard and offer form fill the workspace. `ViewportFrame` offers desktop and phone only, hugs and centres the scaled content, and frames the phone as a device. The offer preview renders inside the same highlights band and three-column grid the signed-in dashboard uses. The approvals desk opens filtered to `pending_approval`, carries per-tab outstanding counts and a workload chip, drops the heading that repeated its own page title, and gives reviewers a preview-led layout. |
| 0 | Sub Admin banner and offer authoring redesign: per-campaign artwork, surface-segregated Media Library, true-scale preview, and separate Banners/Offers pages (direct user instruction; no requirement or completion-percentage change) | **Done - [PR #260](https://github.com/brollysolutions/client1/pull/260)** on `claude/20260829-091848-lets-plan-subadmin-banners-and-offers-righ`. PR #259 shipped a sound data model behind an authoring experience the user rejected on sight. Four defects drove this: the preview clamped `max-width` instead of using the specified fixed 1440/768/390 viewports, so "Desktop" laid the production carousel out at ~480 CSS pixels and fired mobile breakpoints under a label reading "Exact banner preview"; that preview sat inside hand-drawn page furniture (invented nav bar, fake Login pill, three grey skeleton blocks, a hardcoded cream that is not the page background); public artwork was a text dropdown of "label · version" strings and the API returned 422 for any uploaded image outside the dashboard, so "upload from device" was impossible for the homepage, sponsor and section carousels; and dashboard banners and offers had no bundled artwork at all while every library thumbnail was force-cropped to 2:1, making a 9:5 hero and a 16:9 sponsor indistinguishable. | `claude-opus-5` / Extra High (`xhigh`) | `claude-opus-5` / Extra High (`xhigh`) | A campaign takes artwork from exactly one source -- governed category template or a Media Library asset chosen or uploaded for that campaign alone -- with property promotion still template-bound. `campaign_media_assets.usage_type` names one surface each and upload validates the ratio against that surface's target rather than one permissive band. Bundled artwork resolves through `campaign_media.asset_image_url` everywhere, closing a latent defect where bundled-backed campaigns rendered imageless and offers could not be submitted at all. Previews render at real device widths and are scaled to fit, inside `inert` chrome built from the production nav data. `/dashboard/banners` and `/dashboard/offers` are separate pages, each reaching the Media Library as a destination and an in-form picker, with `/dashboard/campaigns` redirecting so pre-split notification links keep resolving. |
| 0 | Sub Admin Campaign Studio, campaign Media Library, and Admin approval desk (direct user instruction; no completion-percentage change) | **Done - [PR #259](https://github.com/brollysolutions/client1/pull/259)** on `codex/20260828-222513-banners-and-offers-are-managed-by-subadmin`. Sub Admin is the team-shared author for banners and authenticated offers, including governed campaign media; Admin has realistic read-only preview, approval, changes-request, and audited soft-removal authority. Provider logos remain in Financial Providers, private operational/customer media stays segregated, and reviewed campaign history is never hard-deleted. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | The additive migration and RLS enforce the maker/checker boundary, team edits use optimistic conflict protection, lifecycle changes notify the original maker, and removed rows cannot serve. The reusable Media Library reports usage and safely deletes only unreferenced assets. Shared production renderers provide desktop, tablet, and mobile previews to both roles. Fresh evidence: API Ruff/format and one Alembic head pass; migration downgrade/upgrade passes; 171 broad changed/adjacent API/RLS tests plus 59 final campaign tests pass; generated contracts are current; web lint/typecheck and all 80 files / 511 tests pass; three focused live-stack Playwright role journeys pass. The full API run reached 1,815 passes / 18 failures; three change-owned ledger failures were fixed in the 171-test pass and the other 15 are unrelated stateful/stale baselines. The Linux production image compiles, typechecks, generates all 93 pages, and completes standalone tracing. Security/design/diff review fixed transaction cleanup, blank-note validation, redundant UI, form association, and picker failure/archive handling; no actionable issue remains. Next priority returns to the FR-2.2 controlled-correction/audit follow-up. |
| 0 | Admin/Sub Admin invitation, staff access, finance, banner, offer, and responsive-dashboard overhaul (direct user instruction; no completion-percentage change) | **Done — [PR #258](https://github.com/brollysolutions/client1/pull/258)** on `codex/20260828-165138-1-agent-invite-modify-agent-invite-ui`. Invite URLs retain their public paths inside the auth shell; staff creation, Staff access, and Operational accounts use responsive floating workspaces and creation immediately attempts a one-use setup link; referral rules use live/retired language and permit only unused retired deletion; Sub Admin finance and payout visibility are maker-only; Admin governs banner media while Sub Admin authors campaigns from approved templates; offers require partner/artwork/terms/code/audience evidence, pass Admin approval, are served only to authenticated Client/Agent/Employee/Telecaller dashboards, and explain external checkout redemption; duplicate authoring pages and the anonymous offers route are removed; shared filters/tables avoid mobile collisions and page-level horizontal scrolling. Historical financial rows, invite-token semantics, maker-checker settlement, public-banner serving, consent, and business-line isolation remain intact. Next priority returns to the formal SRS/coverage queue. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | Fresh evidence: API Ruff/format; migrations downgrade/upgrade and one Alembic head; regenerated OpenAPI/TypeScript contracts; 144 changed/adjacent API, RLS, scheduler, orphan-cleanup, public-catalogue, and authorization tests; web lint/typecheck plus 80 files / 510 unit tests; Linux Docker production build with all 90 pages and standalone traces; eight focused Playwright role/navigation/staff/removed-page/floating-workspace journeys plus manual desktop/mobile review. The aggregate API gate exposed one unrelated stale `test_content_blocks_rls.py` assertion whose expected policy predates checked-in migration `aa12bb34cc56`; the full Playwright command also hits its pre-existing per-IP OTP budget and one unrelated provider-link assertion. Security, design/accessibility, and maintainer review found no remaining change-owned issue. |
| 0 | Admin operations refinements: content-sized loan application dialog, inline filter reset, equal Users & Staff panels, guarded Financial Provider deletion, and reusable approved-Agent setup links (direct user instruction; no requirement or completion-percentage change) | **Done - [PR #257](https://github.com/brollysolutions/client1/pull/257)** on `codex/20260828-123646-1-problem-with-the-floating-window-of`. The loan application detail uses the bounded wide panel; Clear filters participates in the control grid without a dedicated row; provisioning/access panels are equal desktop width and height; Admin can permanently delete only a provider with no application or configured-offer references while referenced providers remain disable-only; and approved Agents awaiting their first password have a paginated Setup links tab with create/copy/share/revoke/reissue controls both immediately after approval and later. Non-goals remained intact: no historical/provider-linked financial data deletion, loan-progression change, Admin password overwrite, automatic external messaging, staff-invite semantic change, or business-line widening. Database `RESTRICT`, Admin/RLS gates, audit events, hash-only expiring/single-use tokens, locked eligibility rechecks, indistinguishable rate-limited invalid tokens, and PII-free audit detail preserve the stated security invariants. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | Fresh evidence: 80 Docker-backed changed-area API/RLS and exhaustive-contract tests; API Ruff check/format; regenerated OpenAPI/TypeScript contracts; one Alembic head; web lint/typecheck and 80 files / 512 tests; production compile/typecheck/90-page generation before the known Windows standalone-symlink `EPERM`; live desktop/mobile Playwright with equal 554px staff panels, inline clear action, 1024x415 loan dialog, loaded Agent setup tab, no 390px page overflow, and no changed-page console errors; security/design/maintainer review. Aggregate API: 1,799 passed / 22 failed on the shared stateful database; three change-owned exhaustive-ledger gaps were fixed and pass in isolation, with the remaining failures outside the diff. Next priority returns to the existing formal SRS/coverage queue. |
| 0 | Admin and Sub Admin dashboard UI overhaul: ten reported defects across floating windows, financial products, listing approvals, staff credentials, assignment queues, and phone entry (direct user instruction; no requirement or completion-percentage change) | **Done** on `feat/dashboard-ui-overhaul`. Every dialog painted `--background` (#F3F3EE) while sheets and inner panels were white, and one shared class forced a 100dvh sheet at thirteen call sites regardless of content; dialogs are white now and content-sized `PANEL_DIALOG_*` variants sit beside the full-screen one, which is kept only where the body genuinely fills it. The financial-product "Public page" tab held the only editable controls for a product, so its catalogue controls moved to a Details tab, the frozen read-only copy was dropped, and the table gained Edit and Activate/Deactivate row actions; there is still no hard delete because historical applications keep the exact form version they were submitted against. Listing approvals splits into Awaiting review and Published tabs (the endpoint already accepted `?status=`), replaces a hand-rolled table whose audit reason came from `window.prompt()`, adds edit and delete to both tabs, and can now withdraw a RERA verification — `not_reviewed` was rejected by both the schema and the service, leaving a mis-verified listing unfixable; withdrawal requires a note, clears the reviewer stamp, and deactivates a live listing. Staff directory rows open a detail dialog carrying the setup-link controls, so skipping the invite at creation no longer strands the account; the temp password is never stored and is stated as unrecoverable, and the deliberate rule that re-issue only works before the user sets their own password is explained rather than surfaced as a raw 409. Access control and its two admin endpoints are withdrawn with the table, service, migration and every runtime consumer retained and the gap recorded in the operational-coverage contract. Lead and Task assignments — the last two admin views on the pre-`DataTable` pattern — are rebuilt on the shared stack, retiring `admin-list-tools`; the sub-admin holdouts are closed; and the two remaining hand-typed E.164 admin fields use the same +91 flag input as every other phone field. No migration, auth, RLS, or money behaviour changed. | `claude-opus-5` / High | `claude-opus-5` / High | Web lint, typecheck, and all 80 files / 512 tests pass. API Ruff passes; the operational-coverage and route-authorization contract tests pass (11); the three RERA integration tests, including two new ones covering note-required withdrawal, reviewer-stamp clearing, catalogue deactivation, and re-verification unblocking approval, pass against the Docker Postgres/Redis stack. The regenerated OpenAPI and TypeScript contracts contain exactly the expected delta: the `/api/v1/admin/field-visibility` path and its three schemas removed, `ReraReviewRequest` re-described. The aggregate API suite and browser verification remain outstanding — the Docker Desktop Linux engine began returning 500 on every request partway through the session, after the RERA tests had passed. |
| 0 | Restore a single Alembic head after the Lead Details and staff-invite migrations merged independently (direct Docker startup failure; no requirement or completion-percentage change) | **Done on `fix/merge-alembic-heads`.** `84a5b6c7d8e9` and `b1f7c93ad204` both descend from `73f4c2a91d6e`, so `alembic upgrade head` refused to select a target and the API could not start. Empty merge revision `c2d8e4f6a901` now names both revisions as parents. Non-goals remain changing either migration, replaying data, altering the staff-invite or loan-transaction contracts, or changing schema/RLS/grants beyond what the two parent migrations already define. | `gpt-5.6-sol` / High | `gpt-5.6-sol` / High | `alembic heads` and the live database both report only `c2d8e4f6a901`. The live startup applied the outstanding staff-invite parent and mergepoint; the pre-existing loan-transaction check constraint and new `staff_invite_links` table are both present. Migration/RLS and Ruff checks pass, and API, scheduler, and web are healthy. Downgrade is deliberately a no-op that only re-exposes the two parent heads. |
| 0 | Concise server-fetch timeout diagnostics (direct user-reported Docker log noise; no requirement or completion-percentage change) | **Done — [PR #245](https://github.com/brollysolutions/client1/pull/245)** on `codex/20260827-065108-the-lead-details-page-ui-its-kinda`. The optional homepage CMS request already failed soft, but logging the raw Node `DOMException` made Next.js print its inherited numeric constants (`INDEX_SIZE_ERR` through `DATA_CLONE_ERR`) and obscure the actionable timeout. The wrapper now logs a bounded exception name/message for network failures without suppressing them; parse-error diagnostics retain their existing stack-bearing behavior. | `gpt-5.6-terra` / Medium | `gpt-5.6-terra` / Medium | Red/green regression proves a timeout is logged as `TimeoutError: The operation was aborted due to timeout` without passing the raw `DOMException`. ESLint, strict typecheck, and all 80 files / 511 web tests pass. The production build compiles, typechecks, and generates 93/93 pages; its intentionally unavailable build-time API calls reproduce concise timeout/fetch diagnostics before the unchanged Windows standalone-symlink `EPERM`. No API, contract, auth/RLS, data, dependency, timeout-policy, or UI behavior changed. |
...
| 0 | Explicit pagination for growing dashboard lists; prohibit endless scroll (direct user instruction; no requirement or completion-percentage change) | **Done — [PR #243](https://github.com/brollysolutions/client1/pull/243)** on `codex/20260827-065108-the-lead-details-page-ui-its-kinda`. No infinite-scroll implementation existed. One accessible, reusable 25-row Previous/Next control now bounds the primary growing Client, Agent, Telecaller, Employee, Sub Admin/Admin-compatible histories and queues that were rendering every fetched row: leads, tasks, vehicle assignments, applications, transactions, support, referrals, commissions/cashbacks, listing submissions, loan media groups, financial-product configuration, document review, and report tables. The helper clamps stale pages after deletion/filtering and affected filters reset to page one. Existing server-paginated audit/catalog flows remain intact; finite detail children, capped previews, and fixed matrices remain deliberately unpaginated. No API, contract, authorization, RLS, ordering, filter, total, or money-transition behavior changed. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | Fresh web lint and typecheck pass; all 80 files / 510 unit tests pass, including four focused pagination tests. The production build compiled, typechecked, and generated 93/93 pages before the established Windows standalone-symlink `EPERM`; unrelated build-time public API fetches also timed out/fell back safely. Source-level design/accessibility review found no actionable issue; live Playwright was limited by an expired local authenticated session (refresh returned 401), and no synthetic 26-row role dataset was added merely for visual proof. Feature tracking and final maintainer review complete before shipping. |
| 0 | Telecaller Lead Details workflow redesign and authoritative validation for calls, loan transactions, progress updates, property terms, and field tasks (direct user-reported defect and UI follow-up; no requirement or completion-percentage change) | **Done — [PR #242](https://github.com/brollysolutions/client1/pull/242)** on `codex/20260827-065108-the-lead-details-page-ui-its-kinda`. Lead Details is workflow-first and responsive, with compact application/deal progress, a transaction table, collapsed submitted answers, a status/call side rail, and visible Phone/WhatsApp labels. Empty and partial new transactions are rejected by accessible web validation and the authoritative API; tasks require instructions; future-time and financial precision/range rules are mirrored across the page and shared Admin progress controls. The additive migration removes only wholly blank legacy transaction rows and installs a narrow database backstop; partial history and all existing assignment, business-line, grant, and RLS behavior remain unchanged. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | Fresh evidence: API Ruff and format pass (473 files); 22 schema tests plus 3 Docker-backed API/RLS tests pass; migration downgrade/upgrade/upgrade passes with exactly one head; generated OpenAPI/TypeScript contracts are current; web lint/typecheck and all 79 files / 506 unit tests pass, with 7 focused validator tests; the production build compiles, typechecks, and generates 93/93 pages before the established Windows standalone-symlink `EPERM`; Playwright verifies desktop/mobile hierarchy, one meaningful history row, labelled contact actions, unique accessible controls, and four focused inline errors with no empty transaction request. The aggregate API suite reached 19% before the unchanged, already-documented Admin coverage-contract failure for `financial_product_provider_offers`; `--lf -x` reproduces that unrelated baseline defect. Security, design/accessibility, and maintainer reviews found no remaining change-owned issue. Non-goals remain new permissions, state-machine changes, or rewriting partial historical rows. |
| 0 | Admin and Sub Admin dashboard UI overhaul, phase 9 of 9: remaining CMS queues, property submissions, and staff Website content removal (direct user instruction; no completion-percentage change pending a formal SRS revision) | **Done** on `claude/20260827-admin-subadmin-ui-foundation`. Banners and offers now use the shared full-width dashboard shell, filter bar, data table, status/list states, pagination, and row-open workspace pattern; Admin offer fields and non-review banner fields are genuinely read-only while existing approval, rejection, archive, replacement, scheduling, audience, and media rules remain intact. My submissions is now a shared table with a centred detail dialog and unchanged edit/withdraw behavior. Banner, referral-rule, audience, and property forms use the shared form-section structure. Per the final direct instruction, Website content management is removed from both Admin and Sub Admin: navigation, dashboard shortcuts, dashboard routes, staff editor/queue/preview code, and their tests are gone. Published public-site content, public rendering, backend content APIs/data, authorization, and lifecycle behavior are retained; historical CMS evidence is therefore superseded only for staff UI availability. | `gpt-5.6-sol` / High | `gpt-5.6-sol` / High | Fresh web lint/typecheck and all 81 files / 515 tests pass. Browser verification covered Admin and Sub Admin at desktop and 390 x 844, including banner/offer/property/referral workspaces, Admin read-only controls, form sections, and both roles receiving 404 from `/dashboard/content`; no mutation was submitted. Root overflow stayed clipped and wide tables retained nested touch/trackpad/programmatic scrolling with hidden horizontal chrome. The normal Docker web service was restored healthy. The requested `design-review` skill was unavailable, so responsive and interaction review was performed manually against the repository primitives and found no remaining change-owned issue. |
| 0 | Admin and Sub Admin dashboard UI overhaul, phase 8 of 9: Analytics, Broadcast, and immutable Audit log (direct user instruction; no requirement or completion-percentage change) | **Done** on `claude/20260827-admin-subadmin-ui-foundation`. Analytics now uses the full-width dashboard shell, shared metric cards, and the shared filter container while retaining its domain-specific weekly/monthly grouping, date presets, multi-Agent selector, sortable `ReportTable`, server pagination/export, team summaries, and request-race guard. Broadcast uses the dashboard shell/panel but preserves both safety gates: Send is disabled until a fresh audience preview exists, audience/line changes invalidate that preview, and the unretractable `window.confirm` names the exact recipient count. Audit log uses the dashboard shell, shared FilterBar/DataTable/list states/pagination, clickable rows, and the existing structured JSON detail dialog; the generated-contract-backed `ACTION_META` remains exhaustive so a new action without display metadata is still a compile error. The filter bar gained optional search/action/clear hooks so reports can reuse it without inventing an irrelevant search box or clearing their required date baseline. No chart, API contract, migration, auth, RLS, report semantics, audit immutability, or broadcast behavior changed. | `gpt-5.6-sol` / High | `gpt-5.6-sol` / High | Fresh web lint/typecheck and all 79 files / 505 tests pass, including report logic and the exhaustive form-surface registry. Playwright covered all three routes at 1440 px and 390 x 844, Analytics metrics/filter/table rendering, an Audit detail row and JSON payload, and Broadcast's disabled-before-preview state plus a 612-recipient confirmation that was dismissed; network evidence contains only the preview request and no send. Document/body widths matched each viewport, mobile Audit retained scroll with hidden horizontal chrome, and no change-owned console error occurred. No chart was introduced, so the plan's `dataviz` skill condition did not apply. |
| 0 | Admin and Sub Admin dashboard UI overhaul, phase 7 of 9: finance ledgers, payout controls, and referral rules (direct user instruction; no requirement or completion-percentage change) | **Done** on `claude/20260827-admin-subadmin-ui-foundation`. Payouts, commissions, processing-fee cashbacks, referral payouts, and referral bonus rules now use the full-width dashboard shell, shared filter/table/status/loading/empty/pagination primitives, and responsive nested scrolling whose horizontal chrome is hidden by the phase-6 global treatment. The near-identical commission and cashback eligible/ledger layouts share one generic `MoneyLedgerView`; their three destination-only payout dialogs share one validation/error implementation while keeping their domain payload builders and APIs separate. A tested pure state helper preserves the load-bearing payable / payout-raised / settled distinction so a pending linked payout cannot be raised twice. Payout approval still exposes review to Admin, shows `viewer_is_maker`, and gates approval on `viewer_can_approve`; manual-cheque issue/clear/fail/reverse and cancel/entry flows are unchanged. Referral rules retains Admin read-only versus Sub Admin create/activate/deactivate authority and its dirty-close confirmation. No API contract, migration, auth, RLS, ledger, or settlement behavior changed. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | Fresh web lint/typecheck and all 79 files / 505 tests pass, including the three-state double-click regression tests and exhaustive form-surface registry. Playwright covered all five routes for Admin at 1440 px and 390 x 844, the maker-checker review dialog without mutation, and the Sub Admin create-rule workspace at 390 x 844. Every document/body width equalled its viewport; every nested horizontal utility reported hidden scrollbar chrome, and mobile wide-table scrollers retained non-zero `scrollLeft`; no change-owned console error occurred. |
| 0 | Admin and Sub Admin dashboard UI overhaul, phase 6 of 9: financial-product catalogue, per-product provider configuration, and app-wide horizontal-scrollbar cleanup (direct user instruction; no requirement or completion-percentage change) | **Done** on `claude/20260827-admin-subadmin-ui-foundation`. Financial products is rebuilt as a filterable/sortable/paginated shared table whose rows open one full-screen workspace with Application form, Providers and Public page tabs. The existing form-version contract is preserved. Public copy is frozen and read-only; only operational product visibility, state, feature and order controls remain editable. Provider operational availability and public offers now meet in the chosen product workspace, with explicit Live on landing page / Draft offer / Operational only / Unavailable states. Live requires a published and verified offer, active/public product and active provider; default operational availability never implies publication. The provider library retains raster-only logo uploads and provenance. The direct follow-up to remove horizontal scrollbar chrome is implemented centrally: horizontal-only overflow utilities hide their bars without disabling touch/trackpad/keyboard/programmatic scrolling, the document clips accidental x overflow, and mixed scrollers retain vertical chrome. No API contract, migration, auth, RLS, or public-copy mutation changed. | `gpt-5.6-sol` / High | `gpt-5.6-sol` / High | Web lint/typecheck and all 78 files / 502 tests pass, including publication-gate coverage. Responsive Playwright/manual review at desktop and 390 x 844 verified the catalogue, three workspace tabs, provider picker/editor, frozen public copy, independent save actions, no document-level x overflow, hidden nested horizontal scrollbar chrome and preserved `scrollLeft`. The requested `apple-design` and `design-review` skills were unavailable, so the visual review used the repository design primitives directly and found no remaining change-owned issue. |
| 0 | Admin and Sub Admin dashboard UI overhaul, phase 5 of 9: tokenised first-login links for provisioned staff (direct user instruction; no requirement or completion-percentage change) | **Done** on `claude/20260827-admin-subadmin-ui-foundation`. Provisioning no longer requires an Admin to relay the generated temporary password: for an active staff identity still in `pending_password_reset`, Admin can mint a seven-day link, copy or share it, revoke it, or replace it (which revokes the prior outstanding link). The public `/staff-invite/{token}` surface reveals only first name and role, lets the holder choose a password under the existing policy, activates the account, and burns the link in the same locked transaction. The new `staff_invite_links` row stores only a SHA-256 token hash plus lifecycle timestamps and identity foreign keys; a partial unique index enforces at most one live link per user, Admin-only RLS and column-scoped UPDATE grants prevent repointing, issue/revoke events are audited without token or PII, and anonymous preview/accept endpoints have separate IP rate-limit budgets while returning one invalid shape for unknown, expired, used, and revoked tokens. The existing forced-reset path and one-time temp-password response remain as the fallback; an invite can never overwrite a password already chosen by the account owner. The generated OpenAPI contract and exhaustive audit-action UI map include the new surface. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | Fresh evidence: all 499 web unit tests, web lint, and web typecheck pass; API Ruff check and format check pass; all 8 focused invite integration tests pass against a migrated isolated Postgres/Redis stack, covering successful login, single use, replacement, revoke/expiry, eligibility, password policy without token burn, Admin-only issue/revoke, and indistinguishable invalid tokens. The named `security-review` skill was unavailable in this session, so a manual security boundary review covered token persistence, public data minimisation, rate limiting, RLS/grants, audit payloads, transaction locking, and eligibility rechecks; it found and fixed the missing reusable Share action and no remaining change-owned security issue. Browser/private-window verification remains for the final interactive pass. |
| 0 | Admin and Sub Admin dashboard UI overhaul, phase 4 of 9: agent applications and the operational account directory (direct user instruction; no requirement or completion-percentage change) | **Done** on `claude/20260827-admin-subadmin-ui-foundation`. Agent applications moves onto the shared table and the workspace dialog, with its four KYC documents rendered as inline previews rather than a list of download links — a photo and an Aadhaar scan are what the decision is made on. The lazy per-open detail fetch is unchanged, since presigned URLs expire in about five minutes. `GET /api/v1/admin/agents` hardcoded `status = pending`, making the queue a one-way door; it now takes a `status` parameter defaulting to `pending`, with an explicit `all` member rather than an empty string, which FastAPI validates against the Literal and rejects. The operational account directory had exactly one filter — a "Search this page" box narrowing only the 25 fetched rows, so an account on page three was unreachable from page one. `GET /api/v1/admin/users` now accepts `search`, `status`, `role`, `business_line`, `created_from`, `created_to` and `never_logged_in`, all applied in the query so `total` stays correct for paging; `role` and `business_line` are EXISTS subqueries so a user with several profiles is counted once, and search covers name, mobile and email against the stored values so a tombstoned mobile cannot resurface a deleted account. The generated spec and typed client are regenerated for the new parameters; no response model changed. | `claude-opus-5` / Extra High (`xhigh`) | `claude-opus-5` / Extra High (`xhigh`) | All 499 web unit tests, web lint, and web typecheck pass. 20 agent and 30 admin-user API tests pass against Postgres, including new coverage for the agent status filter and for server-side user filtering, `total` correctness, and 422 on an invalid status. API Ruff check and format pass. |
| 0 | Admin and Sub Admin dashboard UI overhaul, phase 3 of 9: listing approvals, support tickets, and document verification (direct user instruction; no requirement or completion-percentage change) | **Done** on `claude/20260827-admin-subadmin-ui-foundation`. Listing approvals moves from `features/real-estate/review-queue-view.tsx` to `features/admin/listing-approvals-view.tsx` — it was already an Admin console reaching across for `admin-list-tools` through a relative `../admin/` import — and its review, the richest in the codebase, renders in the full-screen workspace dialog with media on one side and the decision controls on the other. The approve gate is unchanged (RERA settled and every media asset `ready`) but its blocking reason is now stated beside the button instead of hidden in a `title`. Support tickets was the only admin queue with no search, no date range and no pagination; it gains all three plus a category filter, and now passes `status` to the API — the client wrapper and route always accepted it and the hook simply never sent it, so the console was fetching every ticket ever raised. Document verification keeps its boolean model and mandatory un-verify note, but the lead-then-subject card nesting becomes one sortable table with search, review-state, source, line and upload-date filters and real pagination (`business_line` and `offset` were already supported; the wrapper was discarding `total`), and each document now renders inline beside its own note and decision, with a verify-all for the common case. No API contract, migration, auth, RLS, or business-line behavior changed. | `claude-opus-5` / Extra High (`xhigh`) | `claude-opus-5` / Extra High (`xhigh`) | All 499 web unit tests, web lint, and web typecheck pass. The form-surface registry follows the moved listing-approvals module. |
| 0 | Admin and Sub Admin dashboard UI overhaul, phase 2 of 9: loan applications, property deals, and vehicle arrangements rebuilt on the phase-1 primitives (direct user instruction; no requirement or completion-percentage change) | **Done** on `claude/20260827-admin-subadmin-ui-foundation`. Three row-interaction models coexisted across the admin queues — accordion expand, open-a-dialog, and dead rows with side buttons — so whether a row was clickable could not be told by looking. All three of these queues now open a floating window: the full-screen workspace dialog for loan applications and property deals, whose detail is a progress form plus submitted answers, and a centred panel for vehicle arrangements that shows the record read-only alongside the transport form and cancel action. Each view moves off its own `max-w-5xl` wrapper and bare `<h1>` onto `DashboardPage`/`DashboardHeader`/`DashboardPanel`, off a `<ul>` of cards onto `DataTable` with sortable columns, and off its inline filter grid onto the shared `FilterBar` — which adds the business-line filter these records always carried but nothing exposed. Vehicle arrangements was the one admin queue fetching inline in its component; `use-admin-vehicle-arrangements` now matches every sibling, and the previously unsurfaced `driver_mobile`, `completed_at`, `cancelled_at` and `cancellation_reason` appear in its detail panel. No API contract, migration, auth, RLS, or business-line behavior changed. | `claude-opus-5` / Extra High (`xhigh`) | `claude-opus-5` / Extra High (`xhigh`) | All 499 web unit tests, web lint, and web typecheck pass. Loan applications and property deals leave the form-surface registry because their only controls are now the shared filter bar; vehicle arrangements stays registered as a mutation surface. |
| 0 | Admin and Sub Admin dashboard UI overhaul, phase 1 of 9: shared staff-list primitives, app-wide eyebrow removal, one close (X) affordance, and full-width home approval queues (direct user instruction; no requirement or completion-percentage change) | **Done** on `claude/20260827-admin-subadmin-ui-foundation`. Both staff home pages showed their approval queue two or three rows at a time — a fixed-height panel inside an xl-only two-column row, not a data limit. Admin's "Waiting on you" and Sub Admin's "Waiting on Admin" are the same queue seen from the two ends of one approval and now share one full-width table carrying how long each item has waited, with Operational load above it as a four-across strip; the service queue caps rise 12 to 30 and 10 to 30 to match, and the counts beside them stay uncapped. The eyebrow above a page title repeated the sidebar's active section and is removed from `DashboardHeader`, `DashboardFormPage` (where it was required), all 17 call sites, and the two public marketing components; the 404 code and the broadcast Step 1/Step 2 labels stay because they carry information. Close controls had drifted into five treatments and now share `CLOSE_BUTTON_CLASS` — pointer cursor, no border, no background in any state, accent blue on hover — applied in the dialog and sheet primitives and every one-off. New `features/dashboard` primitives (`DataTable`, `FilterBar`, `StatusBadge`, empty/loading/pagination states, `useFilteredPage`, and the workspace dialog moved out of `features/sub-admin`) are what the remaining eight phases build on. No API contract, migration, auth, RLS, or business-line behavior changed. | `claude-opus-5` / Extra High (`xhigh`) | `claude-opus-5` / Extra High (`xhigh`) | All 499 web unit tests, web lint, and web typecheck pass. The 18 admin and sub-admin home API tests pass against a migrated Postgres, covering the raised queue cap. API Ruff check and format pass. Remaining phases: operational queues; listing approvals, support tickets and document verification; agent applications and staff filters; the tokenised staff invite link; financial products; finance; content and insights; and the Sub Admin CMS surfaces. |
| 0 | Application-wide form validation consistency across public/auth surfaces and every Client, Agent, Telecaller, Employee, Sub Admin, and Admin dashboard (direct user instruction; no requirement or completion-percentage change) | **Done — [PR #241](https://github.com/brollysolutions/client1/pull/241)** on `codex/20260826-231901-add-validation-for-all-form-fields-anywher`. A structural registry now classifies all 113 non-primitive input-bearing web surfaces and fails when a future standard, command-search, slider, toggle, or composite field surface is unclassified. Dependency-free typed validators, an accessible inline-error primitive, first-invalid focus, strict dynamic Financial Product schema/answer checks, shared payout-destination validation, bounded filters/calculators, upload checks, and allowlisted sanitized FastAPI 422 locations now cover the previously inconsistent public/auth and role-dashboard forms. The API remains authoritative; no dependency, API, generated contract, migration, auth, RLS, or business-line behavior changed. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | Fresh web lint/typecheck and all 499 unit tests pass. Production build compilation/typechecking and 93-page generation pass before the established Windows standalone-symlink `EPERM`. Two mocked registration/profile/lead Playwright journeys pass; three public catalogue scenarios remain environment-blocked by the unavailable Docker-only `api` hostname (one also hits a pre-existing duplicate-heading strict locator). Repository tracking (7), migration/RLS (11), API Ruff/format, and the single Alembic head pass. The aggregate API run reached 24% before an existing Admin coverage-contract defect (`financial_product_provider_offers` is absent from `ADMIN_OPERATIONAL_COVERAGE`) and unavailable-service setup cascades made it inconclusive; `--lf -x` isolates that unchanged API failure. Security, design, accessibility, and maintainer review found no remaining change-owned issue. Next priority returns to the queued FR-2.2 approved-listing correction and seven append-only audit-event families. |
| 0 | Real-estate browse-card redesign: compact comparison summary with View details as the sole primary action (direct user-approved follow-up; no requirement or completion-percentage change) | **Done — [PR #236](https://github.com/brollysolutions/client1/pull/236).** Playwright first reproduced the 4:3 media, preview sticker, reserved metadata whitespace, and competing details/media/enquiry/site-visit controls, then verified the approved 408px browse card at 1365px and 390px: 16:9 media, subtype, clamped title/location, available-only BHK/area/status facts, price, compact bookmark/compare controls, verified-only RERA corner, and one View details action. View details navigates to the existing page where Enquire and Book a visit remain available; approved media still wins and failed media falls back to local subtype artwork. | `gpt-5.6-terra` / High | `gpt-5.6-terra` / High | Fresh `pnpm lint`, `pnpm typecheck`, focused property-card tests (3/3), and full `pnpm test -- --reporter=dot` (72 files, 454 tests) pass. Playwright MCP verified desktop/mobile layout, verified/unverified treatment, compare state, fallback rendering, and details navigation. The focused Playwright CLI spec was started but produced no terminal result under the resource-starved local Docker stack; equivalent changed-path behavior was exercised directly through Playwright MCP. `pnpm build` compiled, typechecked, and generated 93/93 pages, then hit the repository's established Windows standalone-symlink `EPERM`; Docker-only `api` hostname fetch warnings remained non-fatal during page generation. Design and maintainer review found no actionable issue. No API, contract, migration, auth, RLS, upload, or new external surface changed; next product priority remains unchanged. |
| 0 | Agent dashboard UI overhaul: remove synthetic demo placement banners from the agent home, migrate Leads/Earnings/Introduce-a-lead onto the shared `dashboard-ui.tsx` primitives, add an in-button "Copied" state to the lead-detail registration-link copy action, filter the notification bell's dropdown preview to unread-only so it empties on "mark all as read", and default every remaining application form (public lead dialog, real-estate enquiry/site-visit dialog, dynamic loan-application phone field, agent introduce-a-lead form) to the shared `+91` `MobileInput`, fixing a real paste-with-spaces validation bug on the dynamic loan-application phone field along the way (direct user-reported UI/bug-fix batch; no requirement or completion-percentage change) | **Done — [PR #235](https://github.com/brollysolutions/client1/pull/235)** on `claude/20260826-agent-dashboard-ui-overhaul`, branched fresh from `upstream/main` (a sibling worktree, since the prior task branch had unrelated uncommitted changes left in place per direct user instruction). Seven-item batch: (1) `apps/api/app/scripts/seed_demo.py` — deleted the "Demo Loans workspace"/"Demo Real Estate workspace" `LIVE` banner rows, leaving the unrelated `PENDING_APPROVAL` row used by the Sub Admin approval queue; the agent dashboard's `PersonalizedPlacements` already falls back to real, role-aware copy when no live banner exists. (2) Migrated `agent-leads-view.tsx`, `agent-earnings-view.tsx`, and `agent-introduce-lead-form.tsx` onto `DashboardPage`/`DashboardHeader`/`MetricCard`/`DashboardPanel`/`DashboardFormPage` (Transactions/Agent Home already used these; Leads/Earnings/Introduce-a-lead had not). (3) `agent-lead-detail-view.tsx`'s `copyRegistrationLink` gained a temporary in-button "Copied" state (icon+label swap, `aria-live`), no change to the copied URL itself. (4) `notification-bell.tsx` got a new `selectUnreadPreview()` helper (`lib/notification-state.ts`) so the dropdown preview shows unread items only — the shared snapshot/mutation model and the full `/dashboard/notifications` history page stayed untouched, since a prior PR already solved that page's own "still shows read items" gap via its Unread/All tab; this item is the dropdown-specific follow-up that work didn't cover. (5)-(6) `lead-dialog.tsx`, `property-action-dialog.tsx`, and `agent-introduce-lead-form.tsx` swapped their raw `<Input type="tel">` for the existing `MobileInput` (`components/auth/mobile-input.tsx`) already used by login/register/change-mobile/contact-form/agent-application-form; `financial-product-form.tsx`'s dynamic `input_type: "phone"` field got the same swap plus its `validateProductAnswers` branch changed from a raw regex against the unnormalized string to `isValidMobile()` (`lib/phone.ts`), fixing a genuine bug where a pasted number containing spaces failed validation because the character-count cap and raw-string regex both counted the space. (7) New `lib/phone.test.ts` (did not exist) plus extended `notification-state.test.ts`/`financial-product-form.test.ts` regression coverage. No auth, RLS, payout, migration, or contract-shape change; scope is `apps/web` plus one demo-only `apps/api` seed edit. See `feature-status.md` for the full change-by-change breakdown and verification evidence. | Claude Sonnet 5 / Medium | Claude Sonnet 5 / Medium | Demo banners absent from the agent dashboard (fallback banner shown instead) — met; Leads/Earnings/Introduce-a-lead visually consistent with Transactions/Agent Home — met; copy-registration-link button shows a temporary "Copied" state — met; notification bell dropdown preview empties to an "all caught up" state after "mark all as read" while the full history page is unaffected — met; all listed forms show the `+91` UI and accept a pasted number containing spaces or a `+91` prefix without a false validation error — met. Fresh evidence: `pnpm lint` (clean), `pnpm typecheck` (clean), and `pnpm test` (72 files, 468 tests, all passing) pass; `pnpm build` generated all 93 pages then hit the same pre-existing Windows-host `EPERM` standalone-symlink failure recorded throughout `feature-status.md`, unrelated to this change. Live interactive browser verification was not performed (no running dev container this session) — the residual gap for a follow-up pass. An `apple-design` skill review pass over the six touched agent surfaces found and fixed three issues (a missing required-field indicator/inline error state on the Introduce-a-lead mobile field, a same-row icon-button size mismatch in the notification bell header, a layout-shifting Copy/Copied button) — see `feature-status.md` for the full writeup. |
| 0 | Telecaller leads table and lead detail redesign: migrate both screens onto the shared `dashboard-ui.tsx` primitives, add search/status/follow-up-date filters and sortable columns to the leads table, rebuild the lead detail/call-logging form with a CSS-only call-history timeline, fix a raw-phone-number display bug and a duplicated follow-up-date-field bug along the way (direct user-reported UI overhaul request; no requirement or completion-percentage change) | **Done — [PR #237](https://github.com/brollysolutions/client1/pull/237)** on `claude/20260826-155030-switch-to-main-and-pull-changes`. Closes the gap PR #235 (Agent Dashboard Overhaul) deliberately left open: telecaller's Leads table and lead detail page were the last screens still on the pre-migration hand-rolled layout. `telecaller-leads-view.tsx` migrates onto `DashboardPage`/`DashboardHeader`/`DashboardPanel` and gains an always-visible filter bar (search, status, follow-up date-from/to) plus sortable column headers, backed by new pure `filterTelecallerLeads`/`sortTelecallerLeads` helpers (`telecaller-lead-filters.ts`, unit-tested, mirrors `operational-records-filter.ts`'s shape); default sort is soonest-follow-up-first with never-called leads sinking to the bottom, confirmed with the user over leaving the API's implicit order. No `MetricGrid` added (`telecaller-home.tsx` already shows the same counts and links here) and no business-line filter (the endpoint is already RLS-scoped to the active line per dual-line telecaller). Fixed a latent bug: `use-telecaller-leads.ts` now refetches on `activeLine` change, matching `telecaller-home.tsx`'s existing pattern. `telecaller-lead-detail-view.tsx` is rebuilt on `DashboardPage`/`DashboardBackLink`/`DashboardPanel`/`DashboardFormSection` (one step further than `agent-lead-detail-view.tsx`, which still has no back-link); fixed the phone number rendering raw instead of through `formatMobile()`/`toE164()` (`lib/phone.ts`, which also gained a shared `toWaHref()`), and a "Log a call" bug where the follow-up-date field was rendered on two different DOM nodes bound to the same state depending on the disposition branch. Call history is now a CSS-only vertical timeline. The three business-line sub-sections got an outer-wrapper-only swap onto `DashboardPanel`, and duplicated status/disposition maps were extracted to `telecaller-lead-status.ts`. See `feature-status.md` for the full change-by-change breakdown, including the `apple-design` skill review pass that found and fixed three issues (a low-contrast sort-icon, an undersized sort-header click target, a missing page `<h1>`). No auth, RLS, payout, migration, or contract-shape change; scope is `apps/web` only. | Claude Sonnet 5 / Medium | Claude Sonnet 5 / Medium | Leads table shows working search/status/date filters and sortable columns with soonest-follow-up-first default — met; lead detail page visually consistent with the `DashboardPanel` design language, phone number formatted correctly, follow-up field renders once — met; call history displays as a timeline — met. Fresh evidence: `npm run typecheck` (clean), `npm run lint` (clean), and `npm run test` (73 files, 477 tests, all passing, including two new/extended test files) pass; `npm run build` generated all 93 pages then hit the same pre-existing Windows-host `EPERM` standalone-symlink failure recorded throughout this document, unrelated to this change. Live interactive browser verification was not possible this session — Playwright has no network access at all in this sandbox (confirmed against an external URL, not just localhost); a standalone dev server was started against the already-running API container as a fallback but the browser tool itself could not reach any URL, so an `apple-design` skill source-level review (contrast ratios computed from `globals.css` token values) substituted for a live visual pass — the residual gap for a follow-up verification session. |
| 0 | Clickable-row affordance for Leads/Tasks tables and Employee task screens brought onto `DashboardPanel` (direct user-reported follow-up to PR #237 — "there is no way a telecaller can know to click on leads" plus "even same for employees"; no requirement or completion-percentage change) | **Done — [PR #238](https://github.com/brollysolutions/client1/pull/238)** on `claude/20260826-155030-switch-to-main-and-pull-changes`. PR #237's leads table relied on a bare `hover:bg-muted/50` tint as its only clickability signal; added a `ChevronRight` per row that slides right and tints blue on hover, reusing `dashboard-ui.tsx`'s existing `DashboardQuickAction` micro-interaction verbatim, applied to `telecaller-leads-view.tsx` and (per the "even same for employees" ask) `employee-tasks-view.tsx`. Migrated `employee-tasks-view.tsx`, `employee-task-detail-view.tsx`, `employee-task-document-panel.tsx`, and `employee-task-feedback-panel.tsx` onto `DashboardPage`/`DashboardHeader`/`DashboardBackLink`/`DashboardPanel` — these were untouched by PR #237 and still on the pre-migration hand-rolled layout — and fixed the same raw-phone-display bug there (`formatMobile`/`toE164`). A live Playwright pass over the already-merged PR #237 code (now that this session regained sandbox network access) found no actual rendering defect on the telecaller lead-detail page the user had reported as "broken" — see `feature-status.md` for the full writeup on why an earlier downscaled-screenshot read had been misleading. All functional task-transition/document/feedback logic is untouched. | Claude Sonnet 5 / Medium | Claude Sonnet 5 / Medium | Table rows show an unambiguous clickable affordance on both telecaller and employee list screens — met; Employee task screens visually consistent with the `DashboardPanel` design language — met. Fresh evidence: `npm run typecheck` (clean), `npm run lint` (clean), `npm run test` (73 files, 477 tests, unchanged) pass. This worktree's running `client1-web-1` container bind-mounts the primary checkout, not this worktree, so the chevron/hover affordance and the Employee migration were **not** live-browser-verified this pass (confirmed via `docker exec` that the running container's source lacks these edits) — verified instead via the underlying `DashboardQuickAction`/`DashboardPanel` patterns already live-confirmed under PR #237, plus typecheck/lint/tests. Live verification once merged and pulled is the residual follow-up. No API, contract, migration, or RLS surface changed. |
| 0 | Call/WhatsApp actions converted to icon-only buttons on the telecaller lead detail and employee task detail pages (direct user-reported follow-up — "the UI didn't change for the leads, still same" (stale browser tab, no code fix needed) then "Remove call from web, let it stay on phone, add whatsapp icon and phone icon"; no requirement or completion-percentage change) | **Done — [PR #239](https://github.com/brollysolutions/client1/pull/239)** on `claude/20260826-155030-switch-to-main-and-pull-changes`. A fresh Playwright navigation against the running local stack confirmed PR #238's chevron affordance was already live and correctly rendered — the user's "still same" report was their browser tab holding a pre-restart cached bundle, not a code defect. Converted `telecaller-lead-detail-view.tsx`'s labeled Call/WhatsApp `Button`s (`size="sm"` + text) to icon-only (`size="icon"`, `aria-label`/`title` added) since a `tel:` link is a phone-only affordance and shouldn't present as a primary labeled web action; reused the `size="icon"` `Button` variant already live in eight other files (e.g. `notification-bell.tsx`), not a new pattern. Applied the same icon-only treatment to `employee-task-detail-view.tsx`'s lone Call button for consistency. | Claude Sonnet 5 / Low | Claude Sonnet 5 / Low | Call/WhatsApp render as icon buttons with accessible labels — met. Fresh evidence: `npm run typecheck` (clean), `npm run lint` (clean); no logic changed so `npm run test` is unaffected. Not live-browser-verified this pass — this worktree's running container doesn't yet have the source change (same structural limitation as the previous two entries); confidence rests on `size="icon"` being an already-proven, already-live pattern elsewhere in the app. No API, contract, migration, or RLS surface changed. |
| 0 | Telecaller lead-detail header hero treatment: avatar, status-accent border, entrance animation (direct user-reported follow-up — "I was talking UI changes for the lead detail page", clarifying the earlier "looks broken"/"still same" reports were about this page, not the leads table already fixed in PR #238; no requirement or completion-percentage change) | **Done — [PR #239](https://github.com/brollysolutions/client1/pull/239)** on `claude/20260826-155030-switch-to-main-and-pull-changes`. PR #237's `DashboardPanel` migration was structurally correct but read as too incremental against the original "change the Entire UI" ask. The header card is now hand-composed (DashboardPanel's exact classes copied verbatim for pixel consistency, since its `title` prop is a plain string) so a `UserAvatar` (the same deterministic letter-tile already used for staff identity) can sit beside the lead's name, plus a `border-l-4` status-accent border (new `statusAccentBorderClass()` in `telecaller-lead-status.ts`) and a restrained `animate-in fade-in-0` entrance on the three main panels, reusing the same pattern already proven live on the leads table. No functional change. | Claude Sonnet 5 / Low | Claude Sonnet 5 / Low | Lead detail header shows a visually distinctive avatar + status-accent treatment — met. Fresh evidence: `npm run typecheck` (clean), `npm run lint` (clean), `npm run test` (73 files, 477 tests, unchanged) pass. Not live-browser-verified this pass — same structural limitation as the prior two entries; confidence rests on `UserAvatar` being an already-proven live component and `border-l-4`-over-`border` being a standard Tailwind pattern. No API, contract, migration, or RLS surface changed. |
| 0 | Telecaller lead-detail page rebuilt as a two-column layout, dropping the `max-w-3xl` single-column cap (direct user-reported follow-up after live-verifying the previous hero-treatment entry — "Still same its not full width"; no requirement or completion-percentage change) | **Done — [PR #240](https://github.com/brollysolutions/client1/pull/240)** on `claude/20260826-155030-switch-to-main-and-pull-changes`. The previous entry's avatar/accent/animation polish landed inside a page still capped at `max-w-3xl` — a real, valid complaint distinct from "add more visual flourish." The page now splits into `grid items-start gap-4 xl:grid-cols-[22rem_minmax(0,1fr)]` (the exact grid-template-columns `DashboardFormPage` already uses for its own main+aside layout, reused rather than invented): a 22rem sidebar holds the lead-identity hero card, the flexible main column holds "Log a call"/"Call history". The business-line section and Field tasks stay full width below the grid, unchanged, since their own internal grids need the full page width. Loading/error branches also moved off their own hand-rolled `max-w-3xl` divs onto plain `DashboardPage` calls, so there's no width jump. Below `xl` the grid collapses to a single-column stack as before. | Claude Sonnet 5 / Low | Claude Sonnet 5 / Low | Lead detail page uses the full page width like the rest of the dashboard — met. Fresh evidence: `npm run typecheck` (clean), `npm run lint` (clean), `npm run test` (73 files, 477 tests, unchanged) pass. Not live-browser-verified this pass — same structural limitation as the three entries before it; confidence rests on the grid value being copied verbatim from an already-live layout. No API, contract, migration, or RLS surface changed. |
...
| 6 | Admin operational coverage audit (FR-2.2) | **Done — `feat/admin-financial-enquiry-visibility` — [PR #287](https://github.com/brollysolutions/client1/pull/287):** the second approved slice adds PII-minimized platform-Admin visibility for `financial_service_enquiries` in the existing Operational records workspace. Together with PR #286's read-only `field_visibility_config` oversight, this closes the final registry visibility gap and makes FR-2.2 Complete. The explicitly withdrawn field-policy writer remains a protected non-surface rather than being restored. | `gpt-5.6-sol` / Extra High | `gpt-5.6-sol` / Extra High | The bounded `private, no-store` endpoint uses an explicit-column query and returns only enquiry id, public product label/category, immutable status, form version, and submission time. Client/lead/profile identifiers, form answers, schema snapshots, provider references/snapshots, contact data, and financial facts are neither selected nor serialized. Existing platform-Admin dependencies and request-scoped PostgreSQL RLS remain authoritative; unauthenticated, Client, Sub Admin, and line-scoped Admin requests fail. The read-only generated-contract UI preserves accessible loading/error/empty/pagination states. No mutation, migration, export, workflow progression, field-policy writing, or Client/staff read behavior changed. Fresh evidence includes 12 coverage-contract tests, 10 PostgreSQL authorization/RLS tests, deterministic contracts, API Ruff/format and one head, 7+11 tracking tests, web lint/typecheck plus 95 files / 610 tests, authenticated desktop/mobile Playwright, and a strict Linux 94-route production image. The full API run is 1,946 passes / 4 unrelated failures; an exact rerun passes the transient DNS/reporting cases and reproduces only the known media-environment and polluted-catalogue baselines. Security, design/accessibility, and maintainer review find no change-owned issue. Next priority is the approved-registry publication/re-pull/rescan evidence PR, which remains blocked on a qualified operator and external registry authority. |
...
| --- | --- | --- |
| 2026-09-02 | Corrected the merged registry record: the six Dhanadhara runtime packages exist and deny anonymous access, so deletion and republication are unnecessary. The private runtime publication gate is complete; the next engineering step is a new exact-candidate freeze and rehearsal, while host capacity and external/human launch gates remain open. | [PR #289](https://github.com/brollysolutions/client1/pull/289); raw no-`Authorization` GHCR token requests return HTTP 401 for all six pull scopes and HTTP 200 for a known-public control, authenticated pulls resolve the six recorded OCI-index digests, retained post-publication scan/SBOM hashes and counts match, and all 45 repository script tests pass with one expected Windows POSIX-resource skip. No registry mutation, deployment, credential disclosure, application-code change, or GO decision occurred. |
| 2026-09-02 | Added PII-minimized, read-only platform-Admin oversight of submitted credit-card and insurance enquiries, closing the final FR-2.2 registry visibility gap and moving the 79-requirement implementation ledger to 100%. | [PR #287](https://github.com/brollysolutions/client1/pull/287); explicit-column no-store API/contract/UI projection, 12 coverage-contract tests, 10 focused PostgreSQL authorization/RLS tests, deterministic contracts, API Ruff/format and one head, 7+11 tracking tests, web lint/typecheck/610 tests, authenticated desktop/mobile Playwright, and a strict Linux 94-route image. The full API run completes 1,946 passes / 4 unrelated failures; isolated rerun passes the transient DNS/reporting cases and reproduces the known media-environment and polluted-catalogue baselines. Applicant identifiers, form data, provider snapshots/references, contact data, and financial facts remain outside the Admin query and response; no mutation, migration, export, or Client/staff behavior changed. Security, design/accessibility, and maintainer review find no change-owned issue. |
| 2026-09-02 | Added safe read-only platform-Admin oversight of persisted field-visibility overrides, closing the `field_visibility_config` read gap while leaving `financial_service_enquiries` as the remaining FR-2.2 visibility gap. | [PR #286](https://github.com/brollysolutions/client1/pull/286); minimized no-store API/contract/UI projection, 21 focused PostgreSQL authorization/registry tests, API Ruff/format and one head, deterministic generated contracts, web lint/typecheck/609 tests, focused authenticated Admin Playwright, strict Linux 94-route production image, and security/design/diff review. The full API run's three failures are unrelated baselines; no field-visibility mutation, migration, runtime-default, contact-data, or cross-role behavior changed. |
| 2026-08-31 | Remediated the exact API and isolated media-runtime High/Critical image findings locally without an allowlist, waiver, or loss of the media isolation boundary. | [PR #277](https://github.com/brollysolutions/client1/pull/277); digest-pinned minimal Alpine images `6a95bef6bc24…` and `4aee6affa6fe…` each scan at 0 High/Critical with 123- and 132-component CycloneDX inventories; API secret scan is zero. A fresh isolated Linux run passes 1,948/1,948 API tests, all 504 API files pass style/format, one migration head remains, constrained real media round-trips pass, structural checks are 34 passed / 1 expected Windows skip, and web lint/typecheck/602 tests plus frozen dependency audits pass. Registry publication/re-pull, exact merged-candidate rehearsal, unavailable paid hosted execution, production controls, and all nine human sign-offs remain open. |
| 2026-08-31 | Completed the zero-cost final rehearsal at exact merged PR #275 candidate `fb69226` after the API and production-artifact browser repairs, and retained the NO-GO without waiving unavailable hosted execution or human/environment gates. | [PR #276](https://github.com/brollysolutions/client1/pull/276) and [`release-rehearsal-2026-08-30.md`](release-rehearsal-2026-08-30.md). Fresh terminal evidence includes 1,944/1,944 isolated Linux API tests, 4/4 exact-image browser journeys, 602 web tests and 94-route production build, current secret/dependency/container scans, local production-mode runtime probes, and matching timed synthetic database/object restores. Hosted jobs, API/media findings, registry/external-provider evidence, production controls, and all nine qualified sign-offs remain release blockers. |
| 2026-08-30 | Re-ran the launch rehearsal at exact merged PR #272 candidate `c37b9d5` and retained the NO-GO. Docker recovery allowed the previously interrupted production-artifact, CORS/auth/upload/webhook, scanner/SBOM, aggregate, and synthetic database/object restore work to reach terminal results without self-approving production or human gates. | [PR #273](https://github.com/brollysolutions/client1/pull/273) and [`release-rehearsal-2026-08-30.md`](release-rehearsal-2026-08-30.md). Security-focused API 93/93, web 602 tests plus strict 94-route image, exact runtime probes, and local restores pass. Hosted jobs remain billing-blocked; API/media findings, registry publication, 11 reproducible stale/invalid API tests, Playwright failures/flakiness, positive external upload, production environment controls, and all nine sign-offs remain open. |
| 2026-08-30 | Prepared a canonical NO-GO human/environment sign-off register without treating its creation as approval. Nine stable gates cover trademark, entity/Terms, privacy/processors, DNS/TLS, secrets, monitoring, and incident ownership with accountable roles, required evidence, dated decisions, and expiring remediation-bound risk acceptance. | [PR #269](https://github.com/brollysolutions/client1/pull/269) and [`launch-signoff-register.md`](launch-signoff-register.md); all relative Markdown links resolve, all 7 feature-tracking tests pass, `git diff --check` passes, and security/maintainer review found no actionable issue. Qualified humans still need to provide every approval and environment record. |
| 2026-08-30 | Ran the frozen-candidate rehearsal at `3cc6bc0`, corrected four release defects, and recorded a NO-GO with a strict rerun order rather than treating hosted-runner, Docker-runtime, recovery, image-scan, or human evidence gaps as passing. | [PR #268](https://github.com/brollysolutions/client1/pull/268) and [`release-rehearsal-2026-08-30.md`](release-rehearsal-2026-08-30.md). Fresh completed evidence includes API Ruff/format and one head, 7+11+4 structural/runtime tests, current generated contracts, web lint/typecheck/602 tests/94-route Linux image, full-history Gitleaks plus canary, frozen dependency audits, app SBOMs, web headers/redirect, and exact image-scan counts. |
| 2026-08-30 | Delivered the original FR-2.2 controlled-correction and append-only operational audit remediation, corrected the stale merged-PR #260–#263 plan rows, and promoted the frozen-release rehearsal followed by human launch sign-off as the next delivery phases. | [PR #267](https://github.com/brollysolutions/client1/pull/267); 174 affected API tests, 1,881 aggregate passes with 15 unrelated documented baseline failures and 2 skips, API Ruff/format, migration round-trip/one head, generated contracts, web lint/typecheck/602 tests, strict Linux 94-route production image, authenticated desktop/mobile browser verification, and security/design/maintainer review. No consent banner is appropriate while storage remains limited to essential authentication cookies; Accept/Reject controls are required before non-essential storage is enabled. |
