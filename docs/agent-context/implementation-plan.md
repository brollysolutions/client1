...
| ---: | --- | --- | --- | --- | --- |
| 0 | Sponsor spotlight distinction and responsive creative fitting; section-banner scroll cue; Loans category declutter (FR-2.3, FR-6.1; no completion-percentage change) | **In progress** on `codex/20260819-113333-701-b-out-418-b-xact-25145`: refine the governed homepage sponsor into a clearly disclosed spotlight with restrained, reduced-motion-safe emphasis and source-faithful image presentation; add one functional scroll cue in the existing Financial Services/Properties banner-to-hero gap; remove only the two requested category descriptions and product-count pills. | `gpt-5.6-terra` / High | `gpt-5.6-terra` / High | Preserve the approved template dimensions and public/CMS/API/RLS boundaries. The 960x540 sponsor art must remain legible at phone and desktop layouts; no autoplay or moving copy; the cue must target the overview, work with keyboard navigation, and remain static when reduced motion is requested. Add focused rendering/data tests, verify responsive browser behavior, then update status and PR evidence. |
