...
| ---: | --- | --- | --- | --- | --- |
| 0 | Readable current-location lookup follow-up (FR-17.2-adjacent profile and property-search UX; no completion-percentage change) | **Done — [PR #209](https://github.com/brollysolutions/client1/pull/209), following [PR #208](https://github.com/brollysolutions/client1/pull/208)** on `codex/20260819-165700-lets-work-on-dhanadhara-logo-and-dhanadhar`: the explicit current-location action is the clear Location control beside shared property search; the redundant catalogue selector, all production hardcoded property-location fallbacks, and the Available properties, Saved properties, Cities, and Property categories dashboard metrics are removed. Completion coverage remains 99.4%; next priority remains FR-2.2 controlled correction/audit. | `gpt-5.6-sol` / Extra High (`xhigh`) | `gpt-5.6-sol` / Extra High (`xhigh`) | The authenticated property API is the sole production source for property cards and city/locality/PIN suggestions/filter options. Manual locality/city/PIN/property search, the full Filters sheet, exact live-property matching, two-decimal credential/referrer-free lookup, bounded place fields, actionable failures, provider/IP disclosure, and profile/privacy safeguards remain. Missing configuration leaves a visible disabled action; Docker no longer shadows `.env.local` with an empty value. API/contracts/schema/auth/RLS/deletion/logs/dependencies/business-line isolation are unchanged. Fresh evidence: lint, typecheck, 363 unit tests, two focused 390px Playwright journeys, and responsive visual review pass; the build compiled/typechecked/generated 93 pages before known Windows `EPERM` and unrelated host-side public-data lookup failures. Security/design/maintainer reviews found no actionable defect. |
