...
| ---: | --- | --- | --- | --- | --- |
| 0 | Financial Services and Properties campaign-art composition refresh (FR-12.1-FR-12.3; no completion-percentage change) | **In progress** on `codex/banner-composition-refresh`: replace the 32 governed 1440x576 section-banner WEBPs with distinct editorial scenes that follow the supplied composition reference—clean copy space on the left, scene detail entering the middle, focal subjects primarily on the right—without changing categories, CMS authority, public rendering, APIs, contracts, or RLS. | `gpt-5.6-sol` / High | `gpt-5.6-sol` / High | All 16 Financial Services and 16 Properties assets remain text/logo/watermark-free 1440x576 WEBPs, preserve readable left-side copy contrast, cover every governed category exactly once, and have zero exact duplicate hashes. Validate with the asset contract, contact-sheet review, representative responsive browser crops, full web checks, and design/security/diff review. |
