...
| 6 | Admin operational coverage audit (FR-2.2) | **Partially delivered** on `codex/20260810-161623-ps-d-dhanadhara-client1-docker-compose-f` (PR pending): confirmed CMS override gaps are closed for platform Admin; the next priority is the remaining domain-by-domain inventory. | `gpt-5.6-terra` / High | `gpt-5.6-terra` / High | Inventory every required Admin surface, close confirmed gaps, and add authorization plus accessible UI coverage. |
