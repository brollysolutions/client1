...
| ---: | --- | --- | --- | --- | --- |
| 0 | Financial Services navbar mega-menu (public UI, maintenance — not FR-numbered) | **Done** on `claude/20260817-125827-which-skill-to-use-to-design-frontend`: the public navbar's "Financial Services" item was a plain link to `/loans`, which now carries 16 products (11 loan types, 4 insurance types, credit cards) with no way to browse them from the navbar without landing on the page and scrolling. This reverses the "dropdown children removed, dedicated pages only" decision from commit `45d9573` — see `.agent-workflow/DECISIONS.md` DEC-20260817-01. | `claude-sonnet-5` / Medium | `claude-sonnet-5` / Medium | Desktop renders a 3-column mega-panel (Loans/Insurance/Credit Cards) from a new `components/navbars/financial-services-menu.ts` join layer against `lib/products.ts`; mobile renders the same 3 groups as a native `<details>` disclosure. `lib/products.ts`'s `LOAN_PRODUCTS` grew 7 → 16 with a `group`/`navLabel` field each; `/loans` renders them as two labeled bands (`LOAN_PRODUCT_BANDS`) instead of one flat grid. `property-loan` was retired as a product id, split into `home-loan` (carrying `legacyAnchorId: "property-loan"` so old `/loans#property-loan` links still resolve) and `loan-against-property`; `footer-links.ts`'s `FOOTER_LOAN_IDS` was updated to match. 9 new SVG illustrations were authored in the existing hand-coded style so all 16 `/loans` cards keep the illustrated band (no icon-tile fallback mixed in). `components/ui/navigation-menu.tsx`'s `NavigationMenuViewport` wrapper was recentered so the ~820px panel does not clip off-screen at the 1024px `lg` breakpoint. No route, API, contract, migration, or RLS change. Fresh evidence: `pnpm lint`, `pnpm typecheck`, and `pnpm test` (366/366, including new `lib/products.test.ts` and `components/navbars/financial-services-menu.test.ts`, and updated `nav-items.test.ts`/`footer-links.test.ts`) pass; `pnpm build` compiles, typechecks, and statically generates all 93 pages before the known pre-existing Windows standalone `EPERM` symlink failure. No browser automation tool was connected this session, so the interactive manual pass (panel clipping at 1024/1280/1440px, keyboard walk, focus return, reduced motion, mobile drawer) is unverified — flagged as a residual risk. |
