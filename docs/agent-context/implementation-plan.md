...
| 7 | Analytics completion (FR-16.1 through FR-16.3) | **In progress** — `feat/analytics-completion` | `gpt-5.6-terra` / High | Add formula-safe Excel export and explicit business-line team summaries. The existing multi-Agent selection is the approved ad hoc group filter; do not add an unapproved team-membership model. Preserve bounded async queries, CSV formula safety, pagination, and line isolation. |
