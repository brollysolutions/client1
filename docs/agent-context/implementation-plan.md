...
| --- | --- | --- |
| 2026-08-22 | Delivered the Admin-published Financial Services catalogue, product detail and provider explorer, curated Home services, fixed calculator links, internal Apply/Enquire context, and the reusable verified provider-logo library without external lender redirects or unapproved lender/logo seeds. | PR pending from `feat/configurable-financial-application-forms`; migration round-trip/one head, generated contracts, API Ruff/format, 60 focused API tests plus a 43-test final rerun, 369 web tests, lint/typecheck, two responsive Playwright journeys, visual review, Docker 93-page production build, and security/design/diff review. The monolithic API suite exceeded 40 minutes without a report and is inconclusive. |
