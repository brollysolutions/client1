...
| 1 | Notification unread-state synchronization (FR-17.1) | **Next UI defect:** marking one or all notifications read updates the page list but leaves the shared bell count stale. This is a bounded client-state/cache reconciliation change with no new authorization surface. | `gpt-5.6-terra` / High | `gpt-5.6-terra` / High | The bell, preview, and notification page reconcile immediately after single/all read mutations; failures roll back or refetch; all roles share the same behavior; notification ownership remains enforced by the existing API/RLS boundary. |
