...
| Campaign list/review | `/dashboard/banners`, `/dashboard/offers` | Sub Admin and Admin; actions differ by role |
| Campaign authoring | Floating workspaces on `/dashboard/banners` and `/dashboard/offers` | Sub Admin only |
| Banner media | `/dashboard/banner-media` | Admin only |
...
| ID | Scenario and steps | Expected result |
| --- | --- | --- |
| SA-001 | Open Home with pending approvals, live banner/offer campaigns, and recent referral payouts. | Counts/queue show only authorized campaign-domain data and own pending submissions; no full lead, loan, commission, Client PII, or Admin-only actions appear. |
| SA-002 | Create default, personalized, and action banner drafts for Loans/Real Estate/Both. Select approved Admin media; fill title/subtitle/CTA/deep link/priority/schedule and configure valid audience rules. | Valid draft is created without Sub Admin upload authority. Personalized banners require a supported audience; unknown/incompatible rules fail closed. Deep links must be safe same-origin. |
...
| SA-004 | Create/edit an offer draft with HTTPS partner URL, coupon code, governed artwork, visible terms, dates, priority, and a Client/Agent/Employee/Telecaller audience; submit it and attempt self-approval. | Invalid destinations/date/value/rule combinations fail. Sub Admin cannot self-approve or publish; only Admin-approved offers become eligible for the selected authenticated dashboards, and nothing serves anonymously. |
| SA-005 | Open Finance and inspect summary metrics, referral activity, configured rules, and payout requests as Sub Admins with and without the payout-request grant. | Finance remains a maker overview: historical amounts reconcile, only permitted payout requests appear, and approval, rejection, settlement, and reconciliation controls remain Admin-only. |
| SA-006 | Create referral rules for each line with bonus, minimum conversions, optional cap; retire a rule, then delete an unused retired rule and attempt to delete a referenced rule. | Rules remain concretely line-tagged and conversion-gated. Only retired rules can be deleted, history keeps referenced rules undeletable, and Sub Admin cannot execute referral payouts unless separately using the closed payout-request grant. |
...
| ID | Scenario and steps | Expected result |
| --- | --- | --- |
| AD-C-001 | Review banner campaigns; upload/version governed media; approve/reject a pending banner with reason; attempt campaign authoring as Admin. | Admin owns media and review but cannot author Sub Admin campaigns. Approved schedule/audience governs serving; rejected banners never serve. Removed `/new` routes return 404. |
| AD-C-002 | Review submitted offers; approve or reject with reason; verify approved scheduling/activation/expiry and attempt to publish an unapproved offer. | Only approved campaigns can become live. Audience and business-line rules govern authenticated dashboard serving for Client/Agent/Employee/Telecaller; anonymous serving and unapproved publication remain blocked. |
