...
**Version 1.4 — Final MVP Sign-off (Profile-Based Line Segregation)**
**Date:** 28 June 2026

| | |
|---|---|
| **Document Title** | Loans & Real Estate Platform — Software Requirements Specification (SRS) |
| **Version** | 1.4 — Final MVP Sign-off |
| **Status** | Signed off for MVP Development — Web-Only Scope, Profile-Based Line Segregation |
| **Date** | 28 June 2026 |
...
**Amendment (v1.4 final database segregation).** The system no longer uses `business_line = both` anywhere in the database. A single mobile/login identity is stored in `auth_users`; separate line-specific participation is stored in `client_profiles`, `agent_profiles`, and `staff_profiles`. A customer who wants Loans and Real Estate under one mobile number receives two `client_profiles` rows — one for `loans` and one for `real_estate` — not one mixed record. All business records remain tagged to exactly one line. This supports separate Admin reports for Loan users and Real Estate users while preserving one-login convenience. Agent-introduced leads do **not** auto-expire to an open pool; they remain attributed to the originating agent profile until fulfilled/converted, closed, or manually released by Admin under exception. Client-to-agent conversion keeps the original customer ID/profile immutable and adds an `agent_profiles` row linked to the same `auth_user`. Affected: FR-1.x, FR-3.5, FR-4.3, FR-4.6, Section 5.8, and Section 5.9.
...
The system is a multi-role product. A common login identity layer feeds separate line-specific profile rows and two segregated operational modules (Loans and Real Estate), each with its own dashboards, content, media, and reporting. Routing logic ensures that every lead, inquiry, banner, upload, and report is associated with the correct bucket from the moment it is created, eliminating downstream ambiguity.
...
- **FR-1.6** The system shall not store `business_line = both`. Users who participate in both lines shall have separate line-specific profile rows under the same login identity.
...
- **FR-3.5** Each login identity shall map to one unique mobile number in `auth_users`. A customer may participate in Loans, Real Estate, or both under that mobile number, but the participation shall be stored as separate rows in `client_profiles`. If the customer selects both at registration, the system shall create two profiles: one `loans` profile and one `real_estate` profile. If a customer later adds the missing line, the system shall create the second profile row. No database row shall use `business_line = both`. Internal staff and Agents remain line-specific through `staff_profiles` and `agent_profiles`.
...
- **FR-4.3** A customer shall be able to register directly without an agent, select a requirement — loan, real estate, or both — and be assigned to the relevant telecaller pipeline. If both are selected, the system creates one login identity and two separate client profiles. Each profile follows its own line's workflow and remains independently reportable.
...
- **FR-4.6** Admin shall not routinely transfer an agent-owned lead to a different agent, and agent-owned leads shall not automatically expire to an open pool. An agent-introduced lead remains attributed to the originating `agent_profile` until the client's requirement is fulfilled/converted, the lead is closed, or Admin manually releases the attribution under an exceptional business decision.
...
- **FR-7.4** Employees shall conduct in-person property viewings and collect physical documents where required. Employees collect and upload documents only; Admin verifies internal completeness, while the bank verifies final loan acceptance/sanction where applicable.
...
- **FR-9.5** Referral activity shall be traceable by Admin, with the source preserved. Sub Admin manages referral-bonus rules/content only; Admin executes referral payouts.
...
- **D-2 Lead reassignment by Admin:** the Consolidated PRD implied Admin could change lead ownership, but the Clarification Document states Admin cannot transfer an agent-owned lead to another agent. Resolved in favour of the clarification and the v1.4 decision — unconverted agent leads do **not** auto-expire to an open pool; they remain attributed to the originating agent until fulfilled/converted, closed, or manually released by Admin under exception (FR-4.6).
- **D-3 Single mobile number with separate line profiles:** a mobile number maps to one login identity in `auth_users`. A customer may have a Loan profile and a Real Estate profile under that same login. Selecting both products creates two `client_profiles` rows; adding the missing line later creates the second row. No database row uses `business_line = both`. Internal staff and Agents remain line-specific through `staff_profiles` and `agent_profiles`.

### 5.9 Multi-Line Clients and Agent-Introduced Lead Onboarding

**Decision (profile-based multi-line client).** A single `auth_users` login may operate in the Loans line, the Real Estate line, or both. The database represents this through separate rows in `client_profiles`: one row for `loans` and one row for `real_estate`. The enum value `both` is not used. This allows Admin to report Loan customers and Real Estate customers independently while still allowing one mobile login.

**Decision (client → agent).** When an existing client is approved as an agent, the original client profile/customer ID remains immutable. Agent capability is recorded in a separate `agent_profiles` row linked to the same `auth_user`. The agent profile is line-specific and has its own agent code.
...
| Manage referral-bonus rules/content | ✓ | ✓ | ✗ | ✗ | ✗ | ✗ |
| Execute referral payout | ✓ | ✗ | ✗ | ✗ | ✗ | ✗ |
