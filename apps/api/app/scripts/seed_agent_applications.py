...
No public agent-application submit endpoint exists yet (deferred to a later slice),
so this stands in to make the queue at GET /api/v1/admin/agents demonstrable:
...
real public applicant who has no account yet). Inserts through AsyncSessionLocal
as the `app` superuser (bypasses RLS). Idempotent: skips if pending rows already
exist. Production ships EMPTY — real applications arrive via the public form.
"""
