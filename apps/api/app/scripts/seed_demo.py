...
async def _seed_content(db: AsyncSession, users: dict[str, User], now: datetime) -> None:
    banners = (
        Banner(
            id=demo_uuid("banner:loans-live"),
            business_line="loans",
            placement=BannerPlacement.DASHBOARD,
            banner_type=BannerType.DEFAULT,
            title="Demo Loans workspace",
            subtitle="Synthetic campaign for checking the dashboard placement.",
            cta_label="View applications",
            deep_link="/dashboard/loan-applications",
            audience_rules={},
            priority=100,
            status=BannerStatus.LIVE,
            created_by_uuid=users["sub_admin"].id,
            approved_by_uuid=users["admin"].id,
            starts_at=now - timedelta(days=1),
            ends_at=now + timedelta(days=30),
        ),
        Banner(
            id=demo_uuid("banner:real-estate-live"),
            business_line="real_estate",
            placement=BannerPlacement.DASHBOARD,
            banner_type=BannerType.DEFAULT,
            title="Demo Real Estate workspace",
            subtitle="Synthetic campaign for checking property journeys.",
            cta_label="Explore properties",
            deep_link="/dashboard/explore",
            audience_rules={},
            priority=90,
            status=BannerStatus.LIVE,
            created_by_uuid=users["sub_admin"].id,
