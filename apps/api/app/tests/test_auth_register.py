...
async def test_register_initiate_otp_is_6_numeric_digits(client: AsyncClient) -> None:
    resp = await client.post(
        "/auth/register/initiate",
        json={"first_name": "A", "last_name": "B", "mobile": unique_mobile(), "lines": ["loans"]},
    )
...
async def test_register_initiate_response_shape(client: AsyncClient) -> None:
    ...
    body = resp.json()
    assert "message" in body and "sms_sent" in body and "otp_hint" in body
...
async def test_register_initiate_mobile_without_plus_returns_422(client: AsyncClient) -> None:
    resp = await client.post(
        "/auth/register/initiate",
        json={"first_name": "A", "last_name": "B", "mobile": "9876543210", "lines": ["loans"]},
    )
...
async def test_register_initiate_mobile_too_short_returns_422(client: AsyncClient) -> None:
    resp = await client.post(
        "/auth/register/initiate",
        json={"first_name": "A", "last_name": "B", "mobile": "+91123", "lines": ["loans"]},
    )
    assert resp.status_code == 422
...
async def test_register_initiate_mobile_with_letters_returns_422(client: AsyncClient) -> None:
    resp = await client.post(
        "/auth/register/initiate",
        json={"first_name": "A", "last_name": "B", "mobile": "+91ABCD12345", "lines": ["loans"]},
    )
    assert resp.status_code == 422
...
async def test_register_initiate_empty_lines_returns_422(client: AsyncClient) -> None:
    resp = await client.post(
        "/auth/register/initiate",
        json={"first_name": "A", "last_name": "B", "mobile": unique_mobile(), "lines": []},
    )
    assert resp.status_code == 422
...
async def test_register_initiate_invalid_line_value_returns_422(client: AsyncClient) -> None:
    resp = await client.post(
        "/auth/register/initiate",
        json={"first_name": "A", "last_name": "B", "mobile": unique_mobile(), "lines": ["invalid"]},
    )
    assert resp.status_code == 422
...
async def test_register_initiate_first_name_empty_returns_422(client: AsyncClient) -> None:
    resp = await client.post(
        "/auth/register/initiate",
        json={"first_name": "", "last_name": "B", "mobile": unique_mobile(), "lines": ["loans"]},
    )
    assert resp.status_code == 422
...
async def test_register_initiate_last_name_empty_returns_422(client: AsyncClient) -> None:
    resp = await client.post(
        "/auth/register/initiate",
        json={"first_name": "A", "last_name": "", "mobile": unique_mobile(), "lines": ["loans"]},
    )
    assert resp.status_code == 422
...
async def test_register_initiate_missing_first_name_returns_422(client: AsyncClient) -> None:
    resp = await client.post(
        "/auth/register/initiate",
        json={"last_name": "B", "mobile": unique_mobile(), "lines": ["loans"]},
    )
    assert resp.status_code == 422
...
async def test_register_initiate_missing_last_name_returns_422(client: AsyncClient) -> None:
    resp = await client.post(
        "/auth/register/initiate",
        json={"first_name": "A", "mobile": unique_mobile(), "lines": ["loans"]},
    )
    assert resp.status_code == 422
...
async def test_register_initiate_missing_mobile_returns_422(client: AsyncClient) -> None:
    resp = await client.post(
        "/auth/register/initiate",
        json={"first_name": "A", "last_name": "B", "lines": ["loans"]},
    )
    assert resp.status_code == 422
...
async def test_register_initiate_missing_lines_returns_422(client: AsyncClient) -> None:
    resp = await client.post(
        "/auth/register/initiate",
        json={"first_name": "A", "last_name": "B", "mobile": unique_mobile()},
    )
    assert resp.status_code == 422
...
async def test_register_initiate_duplicate_mobile_returns_400(client: AsyncClient) -> None:
    ...
    resp = await client.post(
        "/auth/register/initiate",
        json={"first_name": "A", "last_name": "B", "mobile": mobile, "lines": ["loans"]},
    )
...
async def test_register_initiate_daily_rate_limit_429_on_sixth(client: AsyncClient) -> None:
    mobile = unique_mobile()
    payload = {"first_name": "A", "last_name": "B", "mobile": mobile, "lines": ["loans"]}
    for _ in range(5):
        await client.post("/auth/register/initiate", json=payload)
    resp = await client.post("/auth/register/initiate", json=payload)
...
async def test_register_verify_correct_otp_returns_token(client: AsyncClient) -> None:
    ...
    resp = await client.post("/auth/register/verify-otp", json={"mobile": mobile, "otp": otp})
    assert resp.status_code == 200
...
async def test_register_verify_wrong_otp_returns_400(client: AsyncClient) -> None:
    ...
    resp = await client.post("/auth/register/verify-otp", json={"mobile": mobile, "otp": "000000"})
    assert resp.status_code == 400
...
async def test_register_verify_missing_mobile_returns_422(client: AsyncClient) -> None:
    resp = await client.post("/auth/register/verify-otp", json={"otp": "123456"})
    assert resp.status_code == 422


async def test_register_verify_missing_otp_returns_422(client: AsyncClient) -> None:
    resp = await client.post("/auth/register/verify-otp", json={"mobile": unique_mobile()})
    assert resp.status_code == 422
...
async def test_register_verify_five_wrong_otps_exhausts_attempts(client: AsyncClient) -> None:
    ...
    await initiate_and_get_otp(client, mobile)
    for _ in range(5):
        await client.post("/auth/register/verify-otp", json={"mobile": mobile, "otp": "000000"})
    resp = await client.post("/auth/register/verify-otp", json={"mobile": mobile, "otp": "000000"})
    assert resp.status_code == 400
...
async def test_register_verify_otp_replay_fails_on_second_use(client: AsyncClient) -> None:
    mobile = unique_mobile()
    otp = await initiate_and_get_otp(client, mobile)
    await client.post("/auth/register/verify-otp", json={"mobile": mobile, "otp": otp})
    resp = await client.post("/auth/register/verify-otp", json={"mobile": mobile, "otp": otp})
    assert resp.status_code == 400
...
async def test_set_password_custom_name_propagated(client: AsyncClient) -> None:
    ...
    resp = await client.post(
        "/auth/register/initiate",
        json={"first_name": "Mahesh", "last_name": "Kumar", "mobile": mobile, "lines": ["loans"]},
    )
    otp = resp.json()["otp_hint"]
    verify_resp = await client.post(
        "/auth/register/verify-otp", json={"mobile": mobile, "otp": otp}
    )
