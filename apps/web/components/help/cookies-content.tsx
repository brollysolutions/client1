...
export function CookiesContent({ dashboard = false }: { dashboard?: boolean }) {
  return (
    <section className="w-full bg-surface-sky px-4 py-10 sm:px-6 sm:py-14">
      <div className="mx-auto max-w-5xl rounded-2xl border border-border bg-surface px-5 py-10 leading-relaxed sm:px-10 sm:py-12">
        <div className="max-w-3xl">
          <p className="font-geist text-sm font-semibold uppercase tracking-[0.16em] text-brand-link">
            Privacy at a glance
          </p>
          <h1 className="mt-3 font-heading text-3xl font-semibold leading-tight tracking-tight text-brand-heading sm:text-5xl">
            Cookie Notice
          </h1>
            ...
            {ESSENTIAL_COOKIES.map((cookie) => (
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <span className="rounded-full bg-brand-cta-tint px-2 py-1 text-[11px] font-semibold text-brand-link">
                    {cookie.category}
                  </span>
                </div>
              ...
              <tbody className="divide-y divide-border text-text-secondary">
                {ESSENTIAL_COOKIES.map((cookie) => (
                  <tr key={cookie.name}>
                    ...
                    >
                      {cookie.name}
                      <span className="mt-2 block w-fit rounded-full bg-brand-cta-tint px-2 py-1 font-heading text-[11px] text-brand-link">
                        {cookie.category}
                      </span>
                    </th>
        ...
        <div className="mt-12 border-t border-[var(--nav-border)] pt-8 text-text-secondary">
          ...
          <p className="mt-3 max-w-3xl leading-7">
            ...
            <Link
              href={dashboard ? "/dashboard/privacy" : "/privacy"}
              className="font-medium text-brand-link underline underline-offset-4"
            >
              Privacy Policy
            </Link>{" "}
            for how personal information is handled, or{" "}
            <Link
              href={dashboard ? "/dashboard/support" : "/contact"}
              className="font-medium text-brand-link underline underline-offset-4"
            >
              contact us
            </Link>{" "}
