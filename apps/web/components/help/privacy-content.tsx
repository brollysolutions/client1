...
function BulletList({ items }: { items: string[] }) {
  return (
    <ul className="mt-4 list-inside list-disc space-y-2 marker:text-brand-link">
      {items.map((item) => (
        <li key={item}>{item}</li>
      ))}
    </ul>
...
export function PrivacyContent({ dashboard = false }: { dashboard?: boolean }) {
  return (
      ...
      <div className="mx-auto max-w-3xl rounded-2xl border border-border bg-surface px-5 py-10 leading-relaxed sm:px-10 sm:py-12">
        ...
        <div className="mt-2 text-base text-text-secondary">
          ...
          <p className="mt-4">
            Our{" "}
            <Link
              href={dashboard ? "/dashboard/cookies" : "/cookies"}
        ...
        <Link
          ...
          className="mt-10 inline-flex h-10 items-center justify-center rounded-md bg-[var(--nav-primary)] px-5 text-sm font-medium text-white transition-colors hover:bg-[var(--nav-primary-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--nav-bg)]"
        >
