    ...
    <>
      ...
      <section className="border-b border-border bg-[var(--nav-bg)]">
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
          <Link href="/loans#financial-services-catalogue" className="inline-flex min-h-11 items-center gap-2 rounded-md text-sm font-medium text-brand-link hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
          ...
          <div className="mt-6 grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_24rem]">
            <div>
              <h1 className="font-heading text-4xl font-semibold text-brand-heading sm:text-5xl lg:text-6xl">{service.label}</h1>
      ...
      <section className="bg-surface px-4 py-14 sm:px-6 sm:py-16 lg:px-8" aria-labelledby="service-enquiry-heading">
        <div className="mx-auto max-w-7xl">
          <h2 id="service-enquiry-heading" className="font-heading text-3xl font-semibold text-brand-heading">Plan your enquiry</h2>
          ...
          <ul className="mt-8 grid gap-5 md:grid-cols-3">
            {guide.topics.map((topic) => (
              <li key={topic} className="flex gap-3 rounded-xl border border-border bg-surface p-5">
                <CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-brand-link" aria-hidden />
          ...
          <div className="mt-8 flex flex-col gap-5 rounded-2xl border border-border bg-[var(--nav-bg)] p-6 sm:flex-row sm:items-center sm:justify-between sm:p-8">
            <div className="flex items-start gap-4">
              <MessageSquare className="mt-1 h-6 w-6 shrink-0 text-brand-link" aria-hidden />
              <div>
                <h3 className="font-heading text-xl font-semibold text-brand-heading">Speak with our team</h3>
      ...
      {related.length > 0 ? (
        <section className="border-t border-border bg-[var(--nav-bg)] px-4 py-14 sm:px-6 sm:py-16 lg:px-8" aria-labelledby="related-services-heading">
          <div className="mx-auto max-w-7xl">
            <h2 id="related-services-heading" className="font-heading text-3xl font-semibold text-brand-heading">More financial services</h2>
            <div className="mt-8 grid gap-5 md:grid-cols-3">
              {related.map((item) => (
                <Link key={item.id} href={financialServiceHref(item.id)} className="group rounded-xl border border-border bg-surface p-6 transition-colors hover:border-brand-blue focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring motion-reduce:transition-none">
                  <div className="flex items-start justify-between gap-3"><h3 className="font-heading text-lg font-semibold text-brand-heading">{item.label}</h3><ArrowRight className="mt-1 h-4 w-4 shrink-0 text-brand-link" aria-hidden /></div>
