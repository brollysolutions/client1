...
import { Bookmark, CalendarCheck, MapPin, Scale } from "lucide-react";
...
// Dashboard variant of the public components/property-card.tsx: same image +
// copy layout, but dashboard blue tokens (not the marketing --nav-* tokens) and
...
// Compact variant for list contexts (bookmarks grid, compare picker) where the
// full card footer isn't needed. Keeps the same visual language.
  ...
  return (
    ...
    >
      <div className="relative hidden h-16 w-16 shrink-0 overflow-hidden rounded-lg bg-loans-soft/50 lg:block">
        {listing.image && (
          <Image src={listing.image} alt="" aria-hidden fill sizes="64px" className="object-cover" />
        )}
      </div>
