...
export function PersonalizedPlacements({
  ...
  return (
    <section aria-label="Dashboard highlights" className="mx-auto mb-6 w-full max-w-[1440px] space-y-3 px-4 sm:px-6 lg:px-8">
      {banners.map((banner) => <DashboardBanner key={banner.id} banner={banner} />)}
      {offers.length ? <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3" aria-label="Offers for you">{offers.map((offer) => <DashboardOffer key={offer.id} offer={offer} />)}</div> : null}
    </section>
  );
}

function DashboardOffer({ offer }: { offer: AuthenticatedOffer }) {
    ...
    <article className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
      ...
      <div className="space-y-3 p-4">
        ...
        <div className="flex items-center justify-between gap-3 rounded-lg border border-dashed border-border px-3 py-2">
          ...
          <Button type="button" variant="ghost" size="sm" onClick={() => void copyCode()} aria-live="polite">{copied ? <Check className="h-4 w-4" aria-hidden="true" /> : <Copy className="h-4 w-4" aria-hidden="true" />}{copied ? "Copied" : "Copy"}</Button>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button asChild size="sm"><a href={offer.redemption_url} target="_blank" rel="noopener noreferrer">Use offer<ExternalLink className="h-4 w-4" aria-hidden="true" /></a></Button>
          {offer.terms_url ? <a href={offer.terms_url} target="_blank" rel="noopener noreferrer" className="text-xs font-medium text-[var(--nav-primary)] underline-offset-4 hover:underline">Full terms</a> : null}
        </div>
...
function DashboardBanner({ banner }: { banner: AuthenticatedBanner }) {
  ...
  return (
    <article className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-brand-navy to-blue-700 p-6 text-white shadow-sm" style={hasImage ? { backgroundImage: `linear-gradient(90deg, rgba(14,35,70,.94), rgba(29,78,216,.72)), url(${JSON.stringify(banner.image_url)})`, backgroundPosition: "center", backgroundSize: "cover" } : undefined}>
      ...
      {hasAction ? <Link href={banner.deep_link!} className="mt-4 inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-semibold text-brand-navy transition-colors hover:bg-blue-50">{banner.cta_label}<ArrowRight className="h-4 w-4" aria-hidden="true" /></Link> : null}
    </article>
  );
}
